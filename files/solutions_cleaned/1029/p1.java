...
class CricketDataHandler {
    ...
    outStream = new PrintWriter(new FileOutputStream("details.csv"));
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player player : players){
    		...
    		int oldRuns = player.getRunsScored();
    				int newRuns = runs + oldRuns;
    		int oldWickets = player.getWicketsTaken();
    		int newWickets = oldWickets + wickets;
    		player.setRunsScored(newRuns);
    		player.setWicketsTaken(oldWickets);
    		
    		}
    		else System.setErr(IllegalArgumentException);
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double totalRunsofTeam = 0;
		double totalPlayersinTeam = 0;
    	for(Player player : players){
    		
    		if(player.getTeamName() == teamName){
    			totalRunsofTeam  = totalRunsofTeam + player.getRunsScored();
    			totalPlayersinTeam++;
    		}
    		else System.err(IllegalArgumentException);
    		
    	}
    	
    	double averageRunsScored = totalRunsofTeam / totalPlayersinTeam;
    	return averageRunsScored;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> requiredList = new ArrayList<>();
for(Player player : players){
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> playersMatchingCriteria = new ArrayList<>();
    	int minRuns = criteria[0];
    	int minWickets = criteria[1];
    	for(Player player : players){
    		if(player.getRunsScored()>minRuns && player.getWicketsTaken()>minWickets){
    			playersMatchingCriteria.add(player);
    		}
    	}
    	return playersMatchingCriteria;
    }
}

public class P2023A7PS0512_P1 {
