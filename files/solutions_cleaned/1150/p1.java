...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner in;
		in= new Scanner("C:\\Users\\user\\Desktop\\SS_24_25_CBT\\SS_24_25_CBT\\inputCricketData.csv");
		in.nextLine();
		while(in.hasNextLine()){
		String n1 = in.nextLine();
		String[] n2 = n1.split(",");
		Role r1 = Role.fromString(n2[1]);
		int run=Integer.parseInt(n2[2]);
		int wicket = Integer.parseInt(n2[3]);
		player_list.add(new Player(n2[0],r1,run,wicket,n2[4]));
		}
		in.close();
		return player_list;
    }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter outstream=null;
    		...
    		outstream.println("Name,Role,Runs,Wickets,TeamName");
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	int count=0;
    	int totalrunscored=0;
    	for(Player i: players){
    		if(i.getTeamName().equals(teamName)){
    			count+=1;
    			totalrunscored=i.getRunsScored();
    		}
    	}
    	return (double)totalrunscored/count;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> all_rounder = new ArrayList<>();
    	for(Player i: players){
    		if(i.getRunsScored()>criteria[0] && i.getWicketsTaken()>criteria[1]){
    			all_rounder.add(i);
    		}
    	}
    	return all_rounder;
