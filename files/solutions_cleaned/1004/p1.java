...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	if(runsScored.p1>runsScored.p2){
    		return -1;
    	}else if(runsScored.p1 == runsScored.p2){
    		return 0;
    	}else{
    		return 1;
    	}
...
class CricketDataHandler {
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	if(playerName.equals(this.playerName)){
    		...
    		this.wicketsTaken += wickets;
    			...
    			throw new IllegalArgumentException("Unknown Player: " + this.playerName);
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(indianPlayer.equals(this.playerName)){
    	int indiaTotal += this.runsScored;	
    	}else if(australianPlayer.equals(this.playerName)){
    		int australiaTotal += this.runsScored;
    	}else{
    		int englandTotal += this.runsScored;
        }
        indiaAvg = indiaTotal/(indianPlayers.size());
        ausAvg = australianTotal/(australianPlayers.size());
        engAvg = englandTotal/(englandPlayers.size());
        return indiaAvg;
        return ausAvg;
        return engAvg;
    }
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
        ...
        if(this.teamName.equals(India)){
        		indianPlayers.add(this.Player);
	    }
        if(this.teamName.equals(Australia)){
    		australianPlayers.add(this.Player);
        }
        if(this.teamName.equals(England)){
    		englandPlayers.add(this.Player);
        }
        return indianPlayers<>;
        return australianPlayers<>;
        return englandPlayers<>;
}
