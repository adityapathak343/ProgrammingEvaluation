...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNextLine()) {
			...
			int runs = Integer.valueOf(st.nextToken());
			int wickets = Integer.valueOf(st.nextToken());
			String team = st.nextToken();
			String name = name1.concat(name2);
			Role r = Role.fromString(role);
			Player p = new Player(name, r, runs, wickets, team);
			list.add(p);
		}
		inputStream.close();
		return list;
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player p : players) {
    		if((p.getPlayerName()).equals(playerName)) {
    			...
    			int new_runs = old_runs + runs;
    			p.setRunsScored(new_runs);
    			int old_wickets = p.getWicketsTaken();
    			int new_wickets = old_wickets + wickets;
    			p.setWicketsTaken(new_wickets);
    			playerFound = true;
    		}
    	}
    	if(playerFound == false) {
    		Exception e = new Exception(IllegalArgumentException);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double average = (totalRuns/countOfPlayers);
    	return average;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> list = new ArrayList<>();
    	Role role = Role.ALL_ROUNDER;
    	for(Player p : players) {
    		if((p.getRole() == role) && (p.getRunsScored() >= criteria[0]) && (p.getWicketsTaken() >= criteria[1])) {
    			list.add(p);
    		}
    	}
    	return list;
