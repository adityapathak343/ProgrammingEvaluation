...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	// UNDERSTANDING OF THE QUESTION: RETURNING -1 IF FIRST PLAYER HAS MORE RUNS. RETURN 1 IF IF SECOND PLAYER HAS MORE RUNS. RETURN ZERO WHEN EQUAL RUNS.
    	...
    	// ALTERNATE UNDERSTANDING IOF THE QUESTION: RETURNING THE DIFFERENCE BETWEEN THE RUNS OF PLAYER 2 AND PLAYER 1 (RUNS OF PLAYER 2 - RUNS OF PLAYER 1)
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		ArrayList<Player> player_list_from_file = new ArrayList<Player>();
		Scanner in;
		String delims = ",";
		int counter = 0;
		try {
			in = new Scanner(new File(inputCricketData.csv));
		}
		catch(FileNotFoundException e) {
			System.out.println("FILE NOT FOUND, GIVE CORRECT NAME");
		}
		...
		while(in.hasNextLine()) {
			if(counter == 0) {
				...
				String player_name = info[0];
				Role role = Role.fromString(info[1]);
				int runs = Integer.parseInt(info[2]);
				int wickets = Integer.parseInt(info[3]);
				String team = info[4];
				
				Player player_a = new Player(player_name, role, runs, wickets, team);
				player_list_from_file.add(player_a);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	String columns = "PlayerName,Role,RunsScored,WicketsTaken,TeamName";
    	out.append(columns)
    	for(int i = 0; i < players.size(); i++) {
    		Player player_b = players[i];
			String player_name = player_b.getPlayerName();
			Role role = player_b.getRole();
			role = role.toString();
			int runs = player_b.getRunsScored();
			int wickets = player_b.getWicketsTaken(); 
			String team = player_b.getTeamName();
			// convert int runs and wickets to strings
			
			
			String complete_player = player_name + "," + role + "," + runs + "," + wickets + "," + team;
			out.append(complete_player);
    	}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i = 0; i < players.size(); i++) {
    		Player player_c = players.get(i);
    		String player_name = player_c.getPlayerName();
    		if(player_name == playerName) {
    			String player_name = player_c.getPlayerName();
    			Role role = player_c.getRole();
    			String team = player_c.getTeamName();
    			Player pl = new Player(player_name, role, runs, wickets, team);
    			players.set(i, p1);
    		}
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	int count = 0;
    	for(int i = 0; i < players.size(); i++) {
    		Player player_d = players.get(i);
    		String team = player_d.getTeamName();
    		if(team == teamName) {
    			summation += Integer.parseInt(player_d.getRunsScored());
    			count++;
    		}
    	}  
    	
    	double average = summation/count;
    	return average;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(int i = 0; i < players.size(); i++) {
    		Player player_e = players.get(i);
    		if(player_e.getTeamName() == teamName) {
    		teamPlayers.add(player_e);
    		}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	ArrayList<Player> teamPlayers = new ArrayList<Player>();
    	Role role_allr;
    	role_allr = role_allr.fromString("ALLROUNDER");
    	
    	
    	for(int i = 0; i < players.size(); i++) {
    		Role role = players.get(i).getRole();
    		if(role == role_allr) {
    			int wickets = players.get(i).getWicketsTaken();
    			int runs = players.get(i).getRunsScored();
    			if(wickets > criteria[1]) {
    				if(runs > criteria[0]) {
