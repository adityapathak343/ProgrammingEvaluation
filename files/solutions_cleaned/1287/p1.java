...
* ID Number:2022B3A70614P
* Name: Deepanpreet Singh Sandhu
* Lab Number: P7
* System Number: 38
***********************************************************/
...
class RunsComparator implements Comparator<Player> {
	/**************************(Done) Q.1 WRITE CODE FOR THIS METHOD *********************************/
    public int compare(Player p1, Player p2) {
    	...
    	int runs_p1= p1.getRunsScored();
    	int runs_p2= p2.getRunsScored();
    	
    	return runs_p2-runs_p1;
    }
}

class CricketDataHandler {
    
	/**************************(Done) Q.2 WRITE CODE FOR THIS METHOD *********************************/
...
//		List <<String,Role,int,int,String>> players = new ArrayList<>();
		List <Player> players = new ArrayList<>();
		
		Scanner playerDataList = new Scanner(new File(fileName));
		
		playerDataList.nextLine();
		
		while(playerDataList.hasNextLine()) {
			String playerData = new String(playerDataList.nextLine());
			String[] parts = playerData.split(",");
			
			Player temp= new Player(parts[0],Role.valueOf(parts[1]),(Integer)parts[2],(Integer)parts[3],parts[4]);
			players.add(temp);
		}
		Player temp= new Player("Deepan",Role.BOWLER,100,100,"India");
		players.add(temp);
		
		playerDataList.close();
	...
	/**************************(Done) Q.3 WRITE CODE FOR THIS METHOD *********************************/
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	Scanner firstline = new Scanner(new File("C:\\Users\\user\\Desktop\\SS_24_25_CBT\\inputCricketData.csv"));
    	PrintWriter write = new PrintWriter(new File(fileName));
    	
    	String[] parts = firstline.nextLine().split(",");
    	
    	write.println(String.format("%s,%s,%s,%s,%s", parts[0],parts[1],parts[2],parts[3],parts[4]));
    	
    	for(Player player: players) {
    		write.println(player.toCsvFormat());
    	}
    	firstline.close();
    	write.close();
	...
	/**************************(Done) Q.4 WRITE CODE FOR THIS METHOD *********************************/
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player player:players) {
    		if(player.getPlayerName()==playerName) {
    			changed=true;
    			player.setRunsScored(runs);
    			player.setWicketsTaken(wickets);
    		}
    	}
    	if(!changed) {
	...
	/**************************(Done) Q.5 WRITE CODE FOR THIS METHOD *********************************/
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	return (totalRuns)/((double)totalPlayers);
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
	...
	/**************************(Done) Q.6 WRITE CODE FOR THIS METHOD *********************************/
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
	...
	/**************************(Done) Q.7 WRITE CODE FOR THIS METHOD *********************************/
