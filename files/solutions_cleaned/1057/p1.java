...
class CricketDataHandler {
	...
	scan = scan.nextLine();
		...
		while(scan.hasNextLine()){
			...
			Player pl = new Player(s1,Player.Role.fromString(stk.nextToken()),Integer.parseInt(stk.nextToken()),Integer.parseInt(stk.nextToken()),stk.nextToken());
			readplayers.add(pl);
		}
		scan.close();
		return readplayers;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter sc = null;
    	try{
    		sc = new PrintWriter(new FileOutputStream(fileName,true));
    	}
    	catch(FileNotFoundException e){}
    	System.out.println("PlayerName Role RunsScored WicketsTaken Team");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i=0;i<players.length;i++){
    		if((players[i].getPlayerName()).equals(playerName)){
    			players[i].setRunsScored(players[i].getRunsScored()+runs);
    			players[i].setWicketsTaken(players[i].getWicketsTaken()+wickets);
    		}
    		else{
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> teamwise = new ArrayList<>;
    	double sum = 0;
    	for(int i=0;i<players.length;i++){
    	if((players[i].getTeamName()).equals(teamName)){
    		teamwise.add(players[i]);
    		sum += players[i].getRunsScored();
    		
    	 }
    	if(teamwise.length == 0){
    		throw IllegalArgumentException;
    	}
    	return (sum/teamwise.length);
}
    }
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> allstats = new ArrayList<>;
    	for(int i=0;i<players.length;i++){
    		if((players[i].getRole).equals(Role.ALL_ROUNDER)){
    			if(players[i].getRunsScored()< criteria[0] && players[i].getWicketsTaken()< criteria[1]){
    				allstats.add(players[i]);
    			}
    		}
    	}
    	return allstats;
