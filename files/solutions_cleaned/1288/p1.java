...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(sc.hasNextLine()){
			...
			Role role = Role.fromString(columns[1]);
			int runs = Integer.parseInt(columns[2]);
			int wickets = Integer.parseInt(columns[3]);
			String teamName = columns[4];
			
			Player py = new Player(name, role, runs, wickets, teamName);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter pw = new PrintWriter(new FileOutputStream("inputCricketData.csv", true));
    	...
    	for(Player player : players) {
    		...
    		String n = player.getPlayerName();
    		String rl = Role.fromString(player.getRole());
    		String rs = Integer.toString(player.getRunsScored());
    		String tn = player.getTeamName();
    		String w = Integer.toString(player.getWicketsTaken());
    		
    		// Step 4: Write each player's information to the file. [1 mark]
    		pw.printf(n + "," + rl + ","+ rs +","+ w +",", tn);
    		pw.println();
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	PlayerFilter<Player> pfob = new TeamFilterStrategy<>();
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player player : py) {
    		...
    		if(player.getRole().equals(Role.ALL_ROUNDER) && player.getRunsScored() >= criteria[0] && player.getWicketsTaken() >= criteria[1]){
...
public class F2022B5A70763P_P1 {
