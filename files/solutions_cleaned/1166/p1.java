...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(sc.hasNextLine()){
			...
			sa = s.split("");
			list.add(new Player(sa[0],(Role)sa[1] ,Integer.parseInt(sa[2]), Integer.parseInt(sa[3]), sa[4]));
		}
		sc.close();
		return list;
    }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	pw.println("PlayerName, Role, RunScored, WicketsTaken, TeamName");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	if(flag == -1) throw new IllegalArgumentException();
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double sum = 0, size;
    	List<Player> p = filter(players, teamName);
    	if(p.size() == 0) throw new IllegalArgumentException();
    	else{
    		size = (double)p.size();
    		Iterator q = p.iterator();
    		while(q.hasNext()){
    			Player pl = (Player)q.next();
        		sum += (double)pl.getRunsScored();
        	}
    	}
    	return sum/size;
    }
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	while(i.hasNext()){
    		Player p = (Player)i.next();
    		if(p.getTeamName() == teamName) list.add(p);
    	}
    	return list;
    }
}
...
public class P2022B3A70568P_P1 {
