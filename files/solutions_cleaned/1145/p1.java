...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner openfileScanner = null;
		try{
			Scanner openfile = new Scanner(new FileInputStream(fileName));
		...
		while(openfile.hasNextLine()){
			...
			Role role = Role.fromString(parts[1].trim());
			int runsScored = parts[2].trim();
			int wicketsTaken = parts[3].trim();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		//Scanner openfile = new Scanner(new FileInputStream("C:\Users\user\Desktop\SS_24_25_CBT\SS_24_25_CBT\inputCricketData.csv"));
    		outPrintWriter = new PrintWriter(new FileOutputStream(fileName));
    	...
    	String l = openfile.nextLine();
    	outPrintWriter.println(l);
    	
        // Step 3: For each player in the list, convert their details to the desired format. [1 mark]
    	String lines =null;
    	while(openfile.hasNextLine()){
    		lines = openfile.nextLine();
    		outPrintWriter.println(" "+parts[0].trim()+" "+parts[1].trim()+" "+parts[2].trim()+" "+parts[3].trim()+" "+parts[4].trim() );
    	}
