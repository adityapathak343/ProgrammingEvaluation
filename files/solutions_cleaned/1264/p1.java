...
class CricketDataHandler {
	...
	while(instream.hasNext()){
		   String playerName;
		     ...
		     int wicketsTaken;
		    String teamName;
		String st = instream.nextLine();
		StringTokenizer sc = new StringTokenizer(st);
		
	while( sc.nextToken() !=null){
		playerName = sc.nextToken();
		role = (Role) sc.nextToken();
		runsScored = sc.nextToken();
		teamName = sc.nextToken();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
		...
		for(Player player : players){
			outstream.println(player.toCsvFormat());
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if (team.size()== 0){
    		throw new IllegalArgumentException("no players found in team " + teamName);
    	}
    	for(Player player: team){
    		total += player.getRunsScored();
    	
    	}
    	average = total/ team.size();
    	return average;
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> player_match  = new ArrayList<>();
