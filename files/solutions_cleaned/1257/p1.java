...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner fScanner = new Scanner(new FileInputStream("inputCricketData.csv"));
		...
		while (fScanner.hasNext()) {
			...
			String nameString = fScanner.next();
			Role role = Role.fromString(fScanner.next());
			int runs = fScanner.nextInt();
			int wickets = fScanner.nextInt();
			String teamString = fScanner.nextLine().substring(1);

			// Step 6: Create a new player using this information. [1 mark]
			Player player = new Player(nameString, role, runs, wickets, teamString);
	...
	public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
		...
		PrintWriter pWriter = new PrintWriter(new FileOutputStream("inputCricketdata.csv"));

		// Step 2: Write the column names as the first line of the file. [1 mark]
		pWriter.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");

		// Step 3: For each player in the list, convert their details to the desired
		// format. [1 mark]
		for (Player player : players) {
			String nameString = player.getPlayerName();
			Role role = player.getRole();
			int runs = player.getRunsScored();
			int wickets = player.getWicketsTaken();
			String teamString = player.getTeamName();
			pWriter.println(nameString + "," + role + "," + runs + "," + wickets + "," + teamString);
		}
	...
	public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
		...
		for (Player player : players) {
			...
			if (player.getPlayerName().equals(playerName)) {
				...
				player.setRunsScored(runs + player.getRunsScored());
				...
				player.setWicketsTaken(wickets + player.getWicketsTaken());
				return;
			}
...
public class P2022B3A70453P_P1 {
