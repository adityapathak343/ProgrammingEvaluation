...
* ID Number: 2023A7PS0589P
* Name:Soham Kudva Karkale
* Lab Number:6016
* System Number:15
***********************************************************/
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			Scanner p = new Scanner(new FileInputStream("inputCricketData.csv"));
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter k = new PrintWriter(new FileOutputStream("details.txt"));
    	String r = p.nextline();
    	k.println(r);
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int c = 0;c<s.size;c++){
    		s.get(c);
    		if(s.get(c).playerName = playerName){
    			s.get(c).runs+=runs;
    			s.get(c).wickets+=wickets;
    		}
    		else{
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	s.filter(List<Player> players, String teamName);
    	if(returns null){
    		...
    		{
    			totalruns=0;
    			totalruns+=List.get(i).runsScored;
    		}
    		averageRuns=totalruns/n;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	Arraylist<Players> f = new Arraylist<>();
    	m=h.size;
    	for(int j=0;j<m;j++){
    		if(h.get(j).teamName = teamName){
    			f.add(h.get(j));
    		}
    		return f;
    	}
