...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Player p;
		p.setPlayerName(result[0]);
		p.setRole((Role)result[1]);
		p.setRunsScored((int)result[2]);
		p.setTeamName(result[3]);
		p.setWicketsTaken((int)result[4]);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	String temp1="playerName role runsScored wicketsTaken teamName";
    	out=new PrintWriter(filename, temp);
        // Step 3: For each player in the list, convert their details to the desired format. [1 mark]
    	String result1;
    	for(int i=0;i<players.size();i++) {
    		result1+=players[i].toString()+"/n";
    	}
        // Step 4: Write each player's information to the file. [1 mark]
    	out=new PrintWriter(filename,result1);
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	while(itr.hasNext()) {
    		Player p1=itr.next();
    		if(p1.getPlayerName().equals(playerName)==true) {
    			p1.setRunsScored(runs+p1.getRunsScored());
    			p1.setWicketsTaken(wickets+p1.getWicketsTaken());
    		}
    		else {
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	while(itr.hasNext()) {
    		Player p2=itr.next();
    		if(p2.getTeamName().equals(teamName)==true) {
    			count++;
    			sum++;
    		}
    	}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> filterTeamsList=new List<Player>();
    	...
    	while(itr.hasNext()) {
    		Player p3=itr.next();
    		if(p3.getTeamName().equals(teamName)==true) {
    			filterTeamsList.add(p3);
    		}
    	}
    	return filterTeamsList;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	List<Player> matchingCriteriaList=new List<Player>();
        // Step 2: Go through each player in the list. [1 mark]
    	Iterator itr=players.iterator();
    	int minruns, minwickets;
    	Player p4=itr.next();
		minruns=p4.getRunsScored();
		minwickets=p4.getWicketsTaken();
    	...
    	while(itr.hasNext()) {
    		...
    		if(p5.getRunsScored()<minruns) {
    			minruns=p5.getRunsScored();
    		}
    		if(p5.getWicketsTaken()<minwickets) {
    			minwickets=p5.getWicketsTaken()
    		}
    	}
    	Iterator itr1=players.iterator();
    	while(itr1.next()) {
    		Player p6=itr.next();
    		if(p6.getRunsScored()==minruns && p6.getWicketsTaken()==minwickets) {
    			matchingCriteriaList.add(p6);
    		}
    	}
        // Step 4: Return the list containing all matching players. [1 mark]
