...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNextLine()) {
			//if count == 0, we ignore the line and go next, then read as normal. will implement after finishing the other questions.
		...
		//step 5-7
		String delims = " ";
		StringTokenizer st= new StringTokenizer(6,delims);
		while(st.hasMoreTokens()) {
			Player p = new Player( st.charAt(0), st.charAt(1), st.charAt(2),st.CharAt(3));
		}
	...
	public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
		...
		try {
		outputStreamName = new PrintWriter(new FileOutputStream("OutputFileName.txt"));
		}
		catch(FileNotFoundException e) {
	...
	public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
		// Question 4: Write code for updating player statistics [Total: 5 marks]
		...
		for(Player each:players) {
			if (each.getPlayerName()==playerName) {
				each.setRunsScored(each.getRunsScored()+runs);
				each.setWicketsTaken(each.getWicketsTaken()+wickets);
			}
	...
	public double calculateTeamAverageRuns(List<Player> players, String teamName) {
		...
		double TeamRuns;
		int NumPlayers=0;
		for(Player bruh:players) {
			if(bruh.getTeamName()==teamName) {
				TeamRuns=TeamRuns+bruh.getRunsScored();
				NumPlayers++;
			}
	    ...
	    double averageRuns;
		averageRuns= TeamRuns/NumPlayers;
		return averageRuns;
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
	...
	public List<Player> filter(List<Player> players, int[] criteria) {
		...
		List<Player> fitCriteria = new ArrayList<>();
		 //step2-3
