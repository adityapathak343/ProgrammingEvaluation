...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			...
			while(sc.hasNextLine()) {
				...
				Role role = Role.fromString(strList[1]);
				int runScored = Integer.parseInt(strList[2]);
				int wicketTaken = Integer.parseInt(strList[3]);
				String teamName = strList[4];
		        // Step 7: Add the new player to the list. [1 mark]
				Player player = new Player(name,role,runScored,wicketTaken,teamName);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) throws IllegalArgumentException  {
    	...
    	TeamFilterStrategy filter = new TeamFilterStrategy();
    	List<Player> teamPlayer = filter.filter(players,teamName);
    	...
    	for (Player player:teamPlayer) {
    		total+= player.getRunsScored();
    	}
        // Step 4: Compute and return the average runs scored. [1 mark]
    	double avg = total/((double)teamPlayer.size());
    	return avg;
    }
}
...
public class P2022B3A70417P_P1 {
