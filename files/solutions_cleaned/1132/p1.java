...
* ID Number: 2023A7PS0668P
* Name: Kavin Shanmugam
* Lab Number: 6017
* System Number: 30
***********************************************************/
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        while (sc.hasNextLine()){
            ...
            Player p = new Player(st.nextToken(),Role.fromString(st.nextToken()),Integer.parseInt(st.nextToken()),Integer.parseInt(st.nextToken()),st.nextToken());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
        ...
        throw new IllegalArgumentException("No players matched the given name");
    }

	/************************** Q.5 WRITE CODE FOR THIS METHOD *********************************/
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
        double acc = 0;
        if (players.size() == 0) throw new IllegalArgumentException("PLayers list cannot be empty");
        for (Player p : players){
            if (p.getTeamName().equals(teamName)){
                acc += p.getRunsScored();
            }
        }
        acc = acc/(players.size());
        return acc;
    }
}
...
public class F2023A7PS0668P_P1 {
