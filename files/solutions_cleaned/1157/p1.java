...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			...
			System.out.println("File inputCricketData not found");
		...
		while(inputStream.hasNextLine())
		{
			...
			Player p = new Player(name, role, runs, wickets, country);
			list.add(p);
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
			...
			System.out.println("error in opening the cricket file");
			System.exit(0);
		}
    	outStream.println("PlayerName\tRole\tRunsScored\tEicketsTaken\tTeamName");
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(list.size()==0)
    		throw new IllegalArgumentException("no players from this team");
...
public class P2022B4A71282P_P1 {
