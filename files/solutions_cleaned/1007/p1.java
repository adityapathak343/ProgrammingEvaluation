...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while (sc.hasNextLine()) {
			sc.nextLine();
			String name = sc.next();
			Role playerRole = valueOf(sc.next());
			int runs = sc.next();
			int wickets = sc.next();
			String team = sc.next();
			Player p1 = new Player(name, playerRole, runs, wickets, team);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	while(litr.hasNext()) {
        	...
        	in.next();
        	out.print(p.getWicketsTaken());
        	in.next();
        	out.println(p.getTeamName());
