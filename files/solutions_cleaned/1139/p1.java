...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	return Intger.compare(p2.getRuns(),p1.getruns() );
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try(BufferedReader br = new BufferedReader(new FileReader(fileName))){
			...
			while ((line=br.readLine())!null){
				...
				String name =data[0];
				String team=data[1];
				int runs=Integer.parseInt(data[2]);
				int wickets=Integer.parseInt(data[3]);
				player.add(new Player(name,team,runs,wickets));
			}
			return player;	
			}
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	List<Player>player=new ArrayList<>();
		try(BufferedWriter bw = new BufferedWriter(new FileReader(filename))){
			bw.write("Names,Teams,Runs,Wickets");
			bw.newLine();
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player player:players){
    		if(player.getName().equals(name)){
    			player.setRuns(players.getRuns()+runs);
    			player.setWickets(players.getWicket()+wickets);
    			found=true;
    			break;
    		}
    		}
    	if(!found){
    		throw new IllegalArgumentException("Player not found"+names);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player>teamPlayers=players.stream().filter(p->p.getTeam()equals(team)).collect(Collector.toList());
    	if(teamPlayers.isEmpty())
    		throw new IllegalArgumentException("no players from specified team:"+team);
    }
    int totalRuns =teamPlayers.stram().mapToInt(player::getRuns)sum();
    return totalRuns/(double)teamPlayers.size();
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	List<Player>PlayersFilter=new ArrayList<>();
    	for(Player player:players){
    		if(getTeams().equals(team)){
    			PlayersFilter.add(players);
    		}
    	}
    	reuturn filteredPlayers;
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	List<Player>PlayersFilter=new ArrayList<>();
    	for(Player player:players){
    		if(player.getRuns()>=minRuns)&&(players.getWickets()>=minWickets){
    			PlayersFilter.add(players);
    		}
    	}
    	reuturn filteredPlayers;
