...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			...
			while(sc.hasNextLine()) {
				...
				String[] s = temp.split(",");
				Player p = new Player(s[0],Role.fromString(s[1]), Integer.parseInt(s[2]), Integer.parseInt(s[3]), s[4]);
				players.add(p);	
			}
			
		} catch (FileNotFoundException e) {
			System.out.println("Error, the file does not exist : " + e);
		} 
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
			...
			String inf = "PlayerName" +"," + "Role,RunsScored,WicketsTaken,TeamName";
			pr.write(inf);
			pr.close();
//			System.out.println(inf);
//			You can run the above line to verify output, if excel doesn't work
			for(Player player : players) {
				String s = player.getPlayerName() + "," + player.getRole() + "," + player.getRunsScored() + "," + player.getWicketsTaken() + "," + player.getTeamName();
				PrintWriter pr1 = new PrintWriter(fos);
				pr1.write(s);
				pr1.close();
...
//    		}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(Player player : pList) {
    		totalRuns = totalRuns + player.getRunsScored();
    	}
    	return totalRuns/pList.size();
    }
}
