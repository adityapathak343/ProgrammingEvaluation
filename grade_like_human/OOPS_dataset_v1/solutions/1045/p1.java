...
class CricketDataHandler{
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		for (int i=0; sc.hasNextLine(); i++) {
			playersList.add(new Player(sc.nextLine().split(",")));
		}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for (Player j : players){
    		if (j.getPlayerName() == playerName) {
    			j.setRunsScored(j.getRunsScored() + runs);
    			j.setWicketsTaken(j.getWicketsTaken() + wickets);
    		}
    		else throw new IllegalArgumentException("No player matches tha given name");
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double avgRuns = 0.0;
    	int count =0;
    	int arePlayersFound = 0;
    	for (Player i : players) {
    		if (i.getTeamName() == teamName) {
    			avgRuns += i.getRunsScored();
    			count++;
    			arePlayersFound = 1;
    		}
    	}
    	avgRuns = avgRuns/count;
    	
    
    	if (arePlayersFound == 1) return avgRuns;
    	else throw new IllegalArgumentException("No players from specified team");
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> filterTeam = new ArrayList<Player>();
    	for (Player i: players) {
    		if (i.getTeamName() == teamName) {
    			players.add(i);
    		}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for (Player i : players) {
    		if (i.getRole()== Role.ALL_ROUNDER && i.getRunsScored() >= criteria[0] && i.getWicketsTaken() >= criteria[1]) {
    			players.add(i);
    		}
    	}
    	return allRounders;
