...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner scan =new Scanner(new FileInputStream("inputCricketData.csv"));
		while(scan.hasNext()) {
			String next=scan.next();
			String[] s=next.split(",");
			String name=s[0];
			Role r=Role.fromString(s[1]);
			
			int runs=Integer.parseInt(s[2]);
			int wick=Integer.parseInt(s[3]);
			String team=s[4];		
			Player p1=new Player(name,r,runs,wick,team);
			l1.add(p1);
			scan.close();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter outputStream=new PrintWriter(new PrintWriter("output.csv"));
    	outputStream.print("PlayerName,Role,RunScored,Wicketstaken,TeamName");
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	ArrayList<Player> l1=new ArrayList<>();
    	for(Player p:players) {
    		if(p.getRole().equals("ALL_ROUNDER") && p.getRunsScored()>=criteria[0] && p.getWicketsTaken()>=criteria[1]) {
    			l1.add(p);
    		}
    	}
    	return l1;
