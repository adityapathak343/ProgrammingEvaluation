...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		String currentlineString = in.nextLine();
		String [] v;
		...
		while(in.hasNextLine())
		{
			...
			currentlineString = in.nextLine();
			v = currentlineString.split(",");
			PlayerName = v[0];
			role = Role.v[1];
			RunsScored =  Integer.parseInt(v[2]);
			WicketsTaken =  Integer.parseInt(v[3]);
			TeamName = v[4];
		...
		List <Player> p_list = new ArrayList<>(p_arr); 
	    return p_list;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for(int i=0; i<n; i++)
    	{
    		...
    		out.println(p.getPlayerName()+","+p.getRole()+","+p.getRunsScored()+","+p.getWicketsTaken()+","+p.getTeamName());
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	int n = players.size();
    	ArrayList<Player> p_arr = new ArrayList<Player>();
    	Player p;
    	int count=0;
    	for(int i=0; i<n; i++)
    	{
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(int i=0; i<n; i++)
    	{
    		...
    		if(p.getTeamName().equals(teamName)) p_arr.add(p);
    	}
