...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	if(p1.getRunsScored() = p2.getRunsScored();
    	return 0);
    	else (return p2.getRunsScored() - p1.getRunsScored());
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner scanner =new Scanner(new File("Inputdata.csv"))
		int j=0;
		while(scanner.hashnextline()) {
			j++;
			String Linestring=scanner.nextline()
					if(j==1) {continue}
			String[] lineDataString=linestring.split(";")
					Player newplayer=new player(linedata[0],Role.fromString(Linedata[1]) , Integer.parseInt(LineData[2]),Integer.parseInt(LineData[3],Integer.parseInt(LineData[4]) 
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	PrintWriter outputPrintWrit=new PrintWriter(new fileOutputStream(fileName))
    			outputPrintWrit.println("playername ,Role ,RunsScored ,WicketsTaken ,TeamName");
    			while[Player player:players]
    					outputPrintWrit.println(player.toCsvformat());}
    	outputPrinteWrit.close;
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	List<PLayer> teamList=new ArrayList<Player>()
    			for(Player x:players)
    				if(x.getPlayerName()==PlayerName) {int findplayers=x;,findd=1;break;};
    				if(find!=1) {throw new IllegalArgumentException("not found");
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	TeamFilterString filterstrat= new TeamfilterString();
    	List<Player> teamList= filterstrat.filter(players,teamNmae);
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	List<Player> bestPlayers=new ArrayList<Players> ()
    			for(Player x: players)
    				if(x.getPlayerName==playerName)
    					x.setRunsScored(runs+x.getRunsScored>criteria[0]&&x.getWicketsTaken()>criteria[1];
    					x.add(bestPlayers());
    					return bestPlayers();
...
public class 2023A7PS0654P_p1.java {
