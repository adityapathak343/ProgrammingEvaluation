...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(in.hasNextLine()) {
			...
			if(details[0].equalsIgnoreCase("PlayerName")) {continue;}
			else{
				System.out.println(details);
				String player_name = details[0];
			System.out.println("hi" + player_name);
			String roles = details[1];
			...
			Player p = new Player(player_name, role, runsScored, wicketsTaken, teamName);
			playerDetails.add(p);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for(int i=0; i<players.size();i++) {
    		...
    		out.println(p.getPlayerName() + "," + p.getRole() + "," + p.getRunsScored() + "," + p.getWicketsTaken() + "," + p.getPlayerName());
    	}
    	out.close();    	
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double numberOfPlayers = 0.0;
    	for(int i=0;i<players.size();i++) {
    		Player p = players.get(i);
    		if(p.getTeamName().equalsIgnoreCase(teamName)) {
    			averageRuns += p.getRunsScored();
    			numberOfPlayers++;
    		}
    	}
    	averageRuns = averageRuns/numberOfPlayers;
    	return averageRuns;
    }
}
