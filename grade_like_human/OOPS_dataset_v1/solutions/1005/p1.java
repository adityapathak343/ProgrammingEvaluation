...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(sc.hasNextLine()){
			...
			Player newPlayer = new Player(info.get(0), Role.valueOf(info.get(1)), convert(info.get(2)), convert(info.get(3)), info.get(4));
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	try(BufferedWriter wr = new BufferedWriter(new FileWriter("inputCricketData.csv"))) {
    		wr.write("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
    		for(Player p : players) {
    			String csv_form = p.toCsvFormat();
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	for(Player p : players) {
    		if(p.getPlayerName() != playerName) {
    			...
    			int curr_run = p.getRunsScored();
    			int curr_wicket = p.getWicketsTaken();
    			p.setRunsScored(curr_run + runs);
    			p.setWicketsTaken(curr_wicket + wickets);
    		}
    	}
...
public class P_2022A8PS1254P_P1 {
