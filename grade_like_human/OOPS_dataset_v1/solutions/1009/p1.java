...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	if(p1.getRunsScored()-p2.getRunsScored()>0)
    		return -1;
    	if(p2.runsScored-p1.runsScored>0)
    		return 1;
    	if(p2.runsScored==p1.runsScored)
    		return 0;
    	else
    	return p2.runsScored-p1.runsScored;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        ArrayList<String> playerNameList= new ArrayList<>();// Step 1: Create an empty list to store player details. [1 mark]
        try(BufferedReader br=new BufferedReader(new FileReader(filename))){// Step 2: Open the specified file for reading data. [1 mark]
        	  String line;
              while((line=br.readLine())!=null){  // Step 4: Read each line one by one until reaching the end of the file. [1 mark]
            	  try{
            		  String playerName =String.parseString(line.trim());// Step 5: Split the line into different pieces of information. [1 mark]
            		  playerNameList.add(playerName);}  // Step 7: Add the new player to the list. [1 mark]
            	  catch(NumberFormatException e){
            		  System.out.println("Skipping the wrong line"+ line);// Step 3: Ignore the first line since it contains the column names. [1 mark]
            	  }
            	  }
	...
	return playerNameArray;}  // Step 9: Return the complete list of players. [1 mark]
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
       double sum=0;
       for(double num: runsScored){
    	   sum+=num;
       }
       return sum/runsScored.length;
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
        // Question 6: Write code for filtering players by team [Total: 5 marks]
    	 ArrayList<String> playerNameList= new ArrayList<>(); // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	 for(String name: playerName ){
    		 if()// Step 2: Go through each player in the players list. [1 mark]
    	 }
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // criteria[0] = minimum runs, criteria[1] = minimum wickets
    	 ArrayList<String> playerNameList= new ArrayList<>(); // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	 for(String name: playerName ) // Step 2: Go through each player in the list. [1 mark]
