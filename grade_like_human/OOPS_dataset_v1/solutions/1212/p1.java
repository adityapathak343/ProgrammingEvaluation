...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner sc=new Scanner(new File("inputCricketData.csv"));
		sc.nextLine();
		while(sc.hasNext()){
			String []parts;
			...
			Role r1 = null;
			r1.fromString(parts[1]);
			int runscored=Integer.parseInt(parts[2]);
			int wickettaken=Integer.parseInt(parts[3]);
			String teamname=parts[4];
			
			Player demo;
			demo=new Player(pname,r1,runscored,wickettaken,teamname);
			p1.add(demo);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	outStream=new PrintWriter(new FileOutputStream("fileName.csv"));
    	outStream.println("PlayerName Column RunsScore WicketsTaken TeamName");
    	for(Player player : players){
    		...
    		outStream.println(name+r1+runs+wickets);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(count==0){
    		...
    		int totalrun=0;
    		for(Player p1 : team){
    			int runs=p1.getRunsScored();
    			totalrun=totalrun+runs;
    		}
    		
    		return totalrun/team.size();
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p1 : players){
    		Role r1=p1.getRole();
    		Role r2 = null;
    		r2.fromString("ALL_ROUNDER");
    		if(r1==r2 && p1.getRunsScored()>=criteria[0] && p1.getWicketsTaken()>=criteria[1]){
    			player.add(p1);
    		}
    	}
...
public class F2023A7PS0639P_P1 {
