...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		List<Player> playas = new ArrayList<>();
		...
		String rol = inputStream.next();
		Role role = Role.fromString(rol); 
		int run = inputStream.nextInt();
		int wkt = inputStream.nextInt();
		String team = inputStream.next();
		Player np = new Player(name, role, run, wkt, team);
		playas.add(np);
		inputStream.close();
		outputStream.close();
		for(int a = 0; a < playas.size(); a++){
			playas.get(a).toString();
		}
		return playas;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter outputStream = new PrintWriter(new FileOutputStream(fileName));
    	outputStream.print("PlayerName ");
    	outputStream.print("Role ");
    	outputStream.print("RunsScored ");
    	outputStream.print("WicketsTaken ");
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	List<Player> filt = new ArrayList<>();
    	for(Player p : players){
    		if(p.getTeamName().equals(teamName)){
    			filt.add(p);
    		}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	List<Player> alr = new ArrayList<>();
        // Step 2: Go through each player in the list. [1 mark]
    	for(Player p : players){
    		if(p.getRole() == Role.ALL_ROUNDER && p.getRunsScored() > 2000 && p.getWicketsTaken() > 100){
    			alr.add(p);
    		}
    	}
...
public class P2023A7PS0565P_P1 {
