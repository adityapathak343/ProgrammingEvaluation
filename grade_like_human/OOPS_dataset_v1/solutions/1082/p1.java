...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	Player s1= (Player)p1;
    	Player s2 = (Player)p2;
    	if(s1.getRunsScored().compareTo(s2.getRunsScored()==0)){
    		return -1;
    	}
    	else{
    		return 1;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		file = new FileInputStream("inputCricketData.csv");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	outputStreamName = new PrintWriter(new FileOutputStream(fileName1));
    	for(int i=1;i<players1.sizeof();i++){
    		outputStreamName.println(players1.get(i));
    	}
    	
    	outputStreamName.println("PlayerName Role RunsScored WicketsTaken TeamName");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	playerName11=playerName;
    	int runs1 = runs;
    	int wik1 = wickets;
    	int prevruns,prevwik;
    	Player s1= (Player)playerName;
    	if(players2.contains(playerName11)){
    		prevruns = s1.getRunsScored();
    		prevruns = prevruns+runs1;
    		s1.setRunsScored(prevruns);
    		prevwik=s1.getWicketsTaken();
    		prevwik=prevwik+wik1;
    		s1.setWicketsTaken(prevwik);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	teamName1=teamName;
    	List<Player> players3 = new ArrayList<>(players);
    	if(players3.contains(teamName1)){
