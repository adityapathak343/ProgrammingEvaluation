...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNextLine()) {
			...
			int runs = st.nextToken();
			int wickets = st.nextToken();
			String teamName = st.nextToken();
			Player player = new Player(name, role, runs, wickets, teamName);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
    		writer = new BufferedWriter(new FileWriter(stringName));
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	try {
    		for(Player player : players) {
    			if(this.getName() == playerName) {
    				int new_runs = this.getRunsScored() + runs;
    				player.setRunsScored(new_runs);
    				int new_wickets = this.getWicketsTaken() + wickets;
    				player.setWicketsTaken(new_wickets);
    			}
    		}
    	}
    	catch(IllegalArgumentException e) {
    		System.out.println("The given argument doesn't match the expected argument type.");
    		e.printStackTrace();
    	}
    }
    	...
    	try {
    		for(Player player : players) {
    			if(player.getTeamName() equals (teamName) {
    				int sum += this.getrunsScored();
    				count++;
    			}
    		}
    		double avg_runs = (double) sum / count;
    	}
    	return avg_runs;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> allRounderList = new ArrayList<>();
    	for(Player player : players) {
