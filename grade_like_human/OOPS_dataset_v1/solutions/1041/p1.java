...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	//Collections.sort(int getRunsScored());
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		public void readFile(String fileName Player player) {		
		}
		...
		in.next().split(" ");
		Player ps = new playerDetails();
		Player.add("Rahul", "batsman", "42", "3" , "mumbaiindians");

		System.out.println("Complete list of players is " + playerDetails.list());
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	int sameTeam [];
    	for(int i = 0; i<args.lenght(sameTeam[]); i++) {
    		sameTeam.iterate();
...
public class F2023A7PS0160P_P1 {
    ...
    public static void main(String[] args) {
        ...
        try {
            // Read data from file
            players = dataHandler.readPlayersFromFile("C:\\Users\\user\\OOPS05\\CBT\\src\\inputCricketData.csv");
        } catch (FileNotFoundException e) {
