...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner sc = new Scanner(new FileInputStream("src/"+fileName));
		...
		while(sc.hasNextLine()) {
			String linedData = sc.nextLine().trim();
			String[] playerDetails = linedData.split(",");
			String name = playerDetails[0];
			int runs = Integer.parseInt(playerDetails[2]);
			int wickets = Integer.parseInt(playerDetails[3]);
			String team = playerDetails[4];
			Role role = Role.fromString(playerDetails[1]);
			Player pl = new Player(name, role, runs, wickets, team);
			playerData.add(pl);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double average = (double)(totalRuns/teamPlayers.size());
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p:players) {
    		if((p.getRole().equals(Role.fromString("ALL-ROUNDER")))&&(p.getRunsScored()>criteria[0])&&(p.getWicketsTaken()>criteria[1])) {
    			allRounders.add(p);
    		}
    	}
