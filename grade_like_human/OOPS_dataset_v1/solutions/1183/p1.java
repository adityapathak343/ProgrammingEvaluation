...
class Player {
    ...
    public Player(String playerName2, String role2, int runsScored2, int wicketsTaken2, String teamName2) {
...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
        ...
        if(p1.getRunsScored() > p2.getRunsScored()) { return -1;}// Return a negative value if the first player has more runs, 
        else { if(p1.getRunsScored() < p2.getRunsScored()) { return 1;}}
        {if(p1.getRunsScored() == p2.getRunsScored()) { return 0;}}// a positive value if the second player has more runs, or zero if they have the same number of runs.
    }
}
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        List<Player> Details = new ArrayList<> ();// Step 1: Create an empty list to store player details. [1 mark]
       try { Scanner sc = new Scanner(new FileInputStream(fileName)); // Step 2: Open the specified file for reading data. [1 mark]
        sc.nextLine();// Step 3: Ignore the first line since it contains the column names. [1 mark]
        while(sc.hasNext())// Step 4: Read each line one by one until reaching the end of the file. [1 mark]
        {
        	...
        	Player newplayer = new Player(PlayerName, Role, RunsScored, WicketsTaken, TeamName);
        ...
        sc.close();// Step 8: Close the file after reading all data. [1 mark]
       } catch (IOException e){
    	   System.err.println(e.getMessage());
       }
        return Details;// Step 9: Return the complete list of players. [1 mark]
    }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        pw.printf("PlayerName"); 
        pw.printf("Role");
        pw.printf("RunsScored");
        pw.printf("WicketsTaken"); 
        pw.printf("TeamName\n");
        
        
        for(Player P:players)
        {pw.printf(P.getPlayerName());
        pw.print(P.getRole());
        pw.print(P.getRunsScored());
        pw.print(P.getWicketsTaken());
        pw.println(P.getTeamName());}
        } catch (IOException e){
     	   System.err.println(e.getMessage());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
        ...
        for(Player P:players)
        {
        	...
        	{
        		int r = P.getRunsScored() + runs;
        		int w = P.getWicketsTaken() + wickets;
        		P.setRunsScored(r);
        		P.setWicketsTaken(w);
        	}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(Player P:players)
        {
    		...
    		{
    			totalruns = totalruns + P.getRunsScored();
    			numplayers++;
    		}
        ...
        double avg;
        avg = totalruns/numplayers; 
        return avg;// Step 4: Compute and return the average runs scored. [1 mark]
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> FilterTeam = new ArrayList<> ();
    	for(Player P:players)
        {
    		if(P.getTeamName().equals(teamName))
    		{
        ...
        return FilterTeam;// Step 4: Return the list containing all matching players. [1 mark]
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> Filter = new ArrayList<> ();
    	for(Player P:players)
        {
        ...
        return Filter;// Step 4: Return the list containing all matching players. [1 mark]
    }
}

public class P2021B5A32512_P1 {
