...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	int p1runs = p1.getRunsScored();
    	int p2runs = p2.getRunsScored();
    	return Integer.compare(p1runs,p2runs) ;
    }
}
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while (File.hasNext()){
			...
			Player tmp = new Player (parts[0], Role.parts[1], (int)parts[2],(int)parts[3], parts[4]);
			PlayerList.add(tmp);
		}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for (Player i : players) {
    		if (i.getPlayerName()== playerName){
    			flag++;
    			int oldRuns = i.getRunsScored();
    			i.setRunsScored(oldRuns+runs);
    			int oldWickets = i.getWicketsTaken();
    			i.setWicketsTaken(oldWickets+runs);
    		}
    	}
    	if (flag == 0) {
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
        // Question 5: W belonging to the specified team. [2 marks]
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> tmpList = new ArrayList<>();
    	for (Player i : players){
    		if (i.getWicketsTaken()== criteria[1] && i.getRunsScored()== criteria[0] && i.getRole()==Role.ALL_ROUNDER){
    			tmpList.add(i);
    		}
    	}
    	return tmpList ;
