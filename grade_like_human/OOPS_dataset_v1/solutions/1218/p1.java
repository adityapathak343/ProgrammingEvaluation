...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        try {
            ...
            String line = read.readLine(); //ignored first
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player currentPlayer : players){
    			if(currentPlayer.getPlayerName().equals(playerName)){
    				nomatch = false;
    				int oldruns = currentPlayer.getRunsScored();
    				currentPlayer.setRunsScored(runs+oldruns);
    				int oldwickets = currentPlayer.getWicketsTaken();
    				currentPlayer.setWicketsTaken(oldwickets+wickets);
    			}
    		
    	}
    	if(nomatch)throw new IllegalArgumentException();
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double runs = totalRuns;
    	double average = runs/number;
    	return average;
