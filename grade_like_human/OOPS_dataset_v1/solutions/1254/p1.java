...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		String[] a= s.split(",");
			...
			Player p = new Player(a[0],Role.fromString(a[1]), (int)Integer.parseInt(a[2]), (int)Integer.parseInt(a[3]),a[4]);
			l.add(p);
		}
		sc.close();
		return l;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter o=null;
    		o = new PrintWriter(new FileOutputStream("inputCricketData.csv"),true);
    	
        o.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
        for(int i =0;i<players.size();i++)
        {
        	String name = players.get(i).getPlayerName();
            Role r = players.get(i).getRole();
            int run = players.get(i).getRunsScored();
            int w = players.get(i).getWicketsTaken();
            o.println(name+","+r+","+","+run+","+w);
        }
       o.close();
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<players.size();i++)
    	{
    	   ...
    	   {
    		   c++;
    		   runs+=(double)(players.get(i).getRunsScored());
    		   p.add(players.get(i));
