...
class CricketDataHandler {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	while(itr.hasNext()){
    		outstream.println( itr.next().getPlayerName()+","+itr.next().getRole()+","+itr.next().getRunsScored()+","+
    		itr.next().getWicketsTaken()+","+itr.next().getTeamName());
    	}
    	outstream.close();
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	ListIterator<Player> itr = players.listIterator();
    	while(itr.hasNext()){
    		if(itr.next().getPlayerName()==playerName){
    			itr.next().setRunsScored(runs);
    			itr.next().setWicketsTaken(wickets);
    		}
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	ListIterator<Player> itr = players.listIterator();
    	double runs = 0;
    	double numOfPlayers = 0;
    	while(itr.hasNext()){
    		Player next = itr.next();
    		if (next.getTeamName()==teamName){
    			filteredList.add(next);
    			runs += next.getRunsScored();
    			numOfPlayers += 1;
    		}
    	}
    	
    	double avg = runs/numOfPlayers;
    	return avg;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	ListIterator<Player> itr = players.listIterator();
    	while(itr.hasNext()){
    		if (itr.next().getTeamName()==teamName){
    			filteredList.add(itr.next());
    		}
    	}
    	return filteredList;
    } 
...
public class P2023A7PS0584P_P1 {
