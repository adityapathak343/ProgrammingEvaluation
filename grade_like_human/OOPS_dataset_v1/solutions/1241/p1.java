...
class CricketDataHandler {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                ...
                Player player = new Player(
                    playerDetails[0],
                    Role.fromString(playerDetails[1]),
                    Integer.valueOf(playerDetails[2]),
                    Integer.valueOf(playerDetails[3]),
                    playerDetails[4]
                );
                players.add(player);
            }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        try {
            ...
            printStream.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
            for (Player player : players) {
                printStream.print(player.getPlayerName());
                printStream.print(",");
                printStream.print(player.getRole());
                printStream.print(",");
                printStream.print(player.getRunsScored());
                printStream.print(",");
                printStream.print(player.getWicketsTaken());
                printStream.print(",");
                printStream.print(player.getTeamName());
                printStream.println();
            }
            printStream.close();
        }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
        ...
        List<Player> team = new TeamFilterStrategy().filter(players, teamName);
        if (team.isEmpty()) {
            throw new IllegalArgumentException();
        }
        ...
        for (Player player : team) {
            sum_runs += player.getRunsScored();
        }
        return ((double) sum_runs) / team.size();
    }
}
...
public class P2023A7PS0659P_P1 {
