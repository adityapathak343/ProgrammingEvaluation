...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(fileReader.hasNext()) {
			...
			Player p = new Player(playerName, role, runsScored, wicketsTaken, teamName);  // playerName, role, runs, wickets, teamName
			players.add(p);
		} 
		fileReader.close(); 
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        // Step 2: Write the column names as the first line of the file. [1 mark] 
	    	fileWriter.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
	    	...
	    	for(Player p : players) {
	    		...
	    		String info = playerName+","+role+","+runsScored+","+wicketsTaken+","+teamName; 
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	while(p.hasNext()) { 
    		...
    		if(pl.getPlayerName().equals(playerName)) { 
    			...
    			int currentRuns = pl.getRunsScored();
    			int currentWickets = pl.getWicketsTaken();
    			
    			pl.setRunsScored(currentRuns + runs); 
    			pl.setWicketsTaken(currentWickets + wickets);
    		}
    	}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	TeamFilterStrategy teamFilter = new TeamFilterStrategy(); 
    	List<Player> playersByTeam = teamFilter.filter(players, teamName); 
    	...
    	if(playersByTeam.size() == 0) {
    		throw new IllegalArgumentException("No players in the specified team.");
    	}
    	...
    	return (totalRuns / playersByTeam.size());
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	while(p.hasNext()) {
    		Player pl = p.next(); 
    		String currentTeam = pl.getTeamName(); 
    		
    		if(currentTeam.equals(teamName)) {
    			playersByTeam.add(pl);
    		}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	List<Player> playersByCriteria = new ArrayList<>();
        // Step 2: Go through each player in the list. [1 mark] 
    	Iterator<Player> p = players.iterator(); 
    	while(p.hasNext()) {
    		Player pl = p.next(); 
    		if(pl.getRole().equals(Role.ALL_ROUNDER)) {
    			if((pl.getRunsScored() >= criteria[0]) && (pl.getWicketsTaken() >= criteria[1])) {
    				playersByCriteria.add(pl);
    			}
    		}
    	}
    	
    	return playersByCriteria;
