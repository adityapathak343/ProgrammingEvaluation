...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			...
			while(in.hasNextLine())
			{
				...
				Role ans=null;
				try{ans = Role.fromString(type);
	              }catch(Exception e)
				{
	            	  System.out.println(e.getMessage());
				}
				Player p = new Player(fullname,ans, runs,wickets, teamname);
				list.add(p);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        // Step 1: Prepare to write data into the specified file. [1 mark]
    	   ...
    	   try{ output = new PrintWriter(new FileOutputStream(fileName));
    	   output.println("PlayerName   Role   RunScored   wicketsTaken  TeamName");
    	   for(Player player :players)
    	   {
    		    output.println(player.getPlayerName()+" "+player.getRole().toString()+" "+player.getRunsScored()+" "+player.getWicketsTaken()+" "+player.getTeamName());
    	   }
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player  player:players)
        {
       	  if(player.getRole().toString().equalsIgnoreCase("ALL_ROUNDER") && player.getRunsScored()>=criteria[0] && player.getWicketsTaken()>=criteria[1])
       	  {
       		   teamlist.add(player);
       	  }
        }
