...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			scanner = new Scanner(new FileInputStream("inputCricketData.csv"));
		} catch(IOException e) {
		...
		while(scanner.hasNextLine()){
			...
			Role role = Role.fromString(line[1]);
			int runs = Integer.parseInt(line[2]);
			int wickets = Integer.parseInt(line[3]);
			String teamname = line[4];
			
			Player player1 = new Player(player, role, runs, wickets, teamname);
			list.add(player1);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for(Player player: players) {
    		out.println(player.toCsvFormat() + "\n");
    	}
    	out.close();
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	} throw new IllegalArgumentException("No such player was found"); 
    } 
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> playerfilter = new ArrayList<>();
    	for(Player p : players) {
    		if(p.getTeamName() == teamName)
    			playerfilter.add(p);
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p: players) {
    		if(p.getRole() == Role.fromString(ALL_ROUNDER) && p.getRunsScored() == criteria[0] && p.getWicketsTaken() == criteria[1]) {
    			allRounders.add(p);
    		}
    	}
    }
