...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner sc = null;
		try {
			sc = new Scanner(new FileInputStream("inputCricketData.csv"));
		...
		while (sc.hasNextLine()) {
			playerData.add(sc.nextLine());
		}
		Iterator<String> iterate = playerData.iterator();
		while(iterate.hasNext()) {
			StringTokenizer st = new StringTokenizer(iterate.next(),",");
			...
			Player p = new Player(name, role, runsScored, wicketsTaken, teamName);
			listOfPlayers.add(p);
		}
		sc.close();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
		...
		Iterator<Player> iterate = players.iterator();
		while (iterate.hasNext()) {
			Player p = iterate.next();
			String name = p.getPlayerName();
			Role role = p.getRole();
			int runsScored = p.getRunsScored();
			int wicketsTaken = p.getWicketsTaken();
			String teamName = p.getTeamName();
			outstream.println(name + "," + role + "," + runsScored + "," + wicketsTaken + "," + teamName);
		}
		outstream.close();
    }
    
	/************************** Q.4 WRITE CODE FOR THIS METHOD  *********************************/
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) throws Exception {
    	...
    	while (iterate.hasNext()) {
    		...
    		if (p.getPlayerName().equals(playerName)) {
    			...
    			p.setWicketsTaken(wickets + p.getWicketsTaken());
    			flip++;
    			break;
    		}
    	}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) throws Exception{
    	...
    	Iterator<Player> iterate = players.iterator();
    	double noOfPlayers = 0;
    	double sumOfRuns = 0;
    	double average = 0;
    	while (iterate.hasNext()) {
    		Player p = iterate.next();
    		if (p.getTeamName().equals(teamName)) {
    			sumOfRuns = sumOfRuns + p.getRunsScored();
    			noOfPlayers++;
    		}
    	}
    	if (noOfPlayers == 0) {
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	Iterator<Player> iterate = players.iterator();
    	List<Player> listOfPlayersFromTeam = new ArrayList<>();
    	while (iterate.hasNext()) {
    		Player p = iterate.next();
    		if (p.getTeamName().equals(teamName)) {
    			listOfPlayersFromTeam.add(p);
    		}
    	}
    	return listOfPlayersFromTeam;
    }
}
