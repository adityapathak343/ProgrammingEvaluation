...
class Player {
	...
	public static Player parsePlayer(Player line) {
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
	        ...
	        try {
	            ...
	            // Read file line by line and parse into doubles
	            while (reader.hasNextLine()) {
	                String line = reader.nextLine().trim();
	                if (!line.isEmpty()) { // Ensure the line is not empty
	                    Player playeri = Player.parseString(players);
	                    playerdetails.add(playeri);
	                }
	            }
	            reader.close();
    	 ...
    	 for(Player p : players)
      	{
	        ...
	        Player[] displaynewdetails = new Player[newlist.size()];
	        for (int i = 0; i < newlist.size(); i++) {
	        	displaynewdetails[i] = newlist.get(i);
	        }
	        for(Player p : displaynewdetails)
	        {
...
public class CBT_PART_1_QP_2022A3PS1203 {
