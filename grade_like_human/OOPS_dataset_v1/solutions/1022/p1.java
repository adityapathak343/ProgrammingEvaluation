...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
		...
		sort.runscored();
		if runscored.player p1>runscored.player p2
		return -1;
		else if runscored.player p2>runscored.player p1
		return 1;
		else
		return 0;
...
class CricketDataHandler {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        // Step 4: Write each player's information to the file. [1 mark]
		 int (inputCricketData.csv,jasprit bumrah)
		 int (input crirketdata.csv,adam zampa)
		 int(input crirketdata.csv,pat cummins)
		 int(input crirketdata.csv,jofra archer)
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
		...
		if currentplayer name==playername.list;
		{
		try
		update player.argumentruns;
		runs=argumentruns+runs;
		return runs;
		update player.argumentwicket;
		wicket=argumentwicket+wicket;
		return wicket;
		else
		throws illegalargumnetexception(exception e);
		}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
		...
		sortTeamName;
		if p1.teamname==p2.teamaname;
		return p1.runs;
		return p2.runs;
		total runs=runs.teamname;
		avg runs=runs.teamname/no of team;
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
		...
		list mylist;
		mylist=(list0malloc(sizeof(struct linked_list));
		mylist->count=0;
		mylist->head=null;
		return mylist;
    }
