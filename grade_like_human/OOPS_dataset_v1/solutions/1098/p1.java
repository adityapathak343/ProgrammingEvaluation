...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			String playerName = splitDetails[0];
		    Role role = Role.fromString(splitDetails[1]);
		    int runsScored = Integer.parseInt(splitDetails[2]);
		    int wicketsTaken = Integer.parseInt(splitDetails[3]);
		    String teamName = splitDetails[4];
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	try {
    		for(Player player : players) {
    			if(player.getTeamName() == teamName) {
    				totalTeamRuns += player.getRunsScored();
    				teamPlayers += 1;
    			}
    		}
    	} catch (IllegalArgumentException e) {
    	...
    	if(teamPlayers > 0) {
    		double averageRuns = totalTeamRuns/teamPlayers;
        	return averageRuns;
    	} else {
    		return 0.0;
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> matchCriteriaPlayers = new ArrayList<>();
    	
    	for (Player player: players) {
    		if (player.getRole() == Role.fromString("all-rounder") && player.getRunsScored() >= criteria[0] && player.getWicketsTaken() >= criteria[1]) {
    			matchCriteriaPlayers.add(player);
    		}
    	}
    	
    	return matchCriteriaPlayers;
    }
}

public class P2022B3A70525_P1 {
