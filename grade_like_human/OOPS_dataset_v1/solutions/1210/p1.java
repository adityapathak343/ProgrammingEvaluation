...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
	    ...
	    role.getRole();
	    Player runsScored;
	    runsScored.getRunsScored();
	     
	    Player wicketsTaken;
	    wicketsTaken.getWicketsTaken();
	     Player teamName;
	     ...
	     for (int i = 0; i < playerDetails.length() ;i++) {
	      ...
	      String playerName1 = parts[0].trim();
	    String Role1 = parts[1].trim();
	      int runsScored1 = Integer.parseInt(parts[2].trim());
	      int wicketsTaken1 = Integer.parseInt(parts[3].trim());
	      String teamName1= parts[4].trim();	      
	     }
	     
	     
	    try {
	    	// playerDetails<>() = (playerName1, Role1, runsScored1, wicketsTaken1,  teamName1);
	    	 Player player = new Player(String playerName, Role role, int runsScored, int wicketsTaken, String teamName);
	    	 playerDetails.add(playerName1, Role1, runsScored1, wicketsTaken1,  teamName1);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try (PrintWriter write = new PrintWriter("out.cvs")){
    		write.println("playerName" );
    		write.println("role ");
    		write.println("RunsScored ");
    		write.println("wickectsTaken ");
    		write.println("TeamName ");
    		List<Player> player = new List<String, Role, Integer, Integer, String>();
    	...
    	try {
    		...
    		for(int i = 0; i<list.size(); i++ ) {
    			if(player.getPlayerName() == playerName ) {
    				sum1 = runs + player.getRunsScored();
    				player.setRunsScored(sum1);
    				sum2 = wickets + player.getWicketsTaken();
    				player.setWicketsTaken(sum2);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	try {
    		List<Player> players = teamName.stream().filter()
    	}
    	catch(IllegalArgumentException e) {
    		System.out.println("No same teams");
    	}
    }
}
...
public class P2023A7PS0507P_P1 {
