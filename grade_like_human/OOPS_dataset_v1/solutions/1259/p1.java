...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		public list<Player> PlayerDeatils=new ArrayList<>();
		...
		while((line=reader.readLine()!=null)){
			...
			System.out.println("reading each line in the file");
		}
		Player newPlayer=new Player(line.toCsvFormat());
		PlayerDetails.add(newPlayer);
		reader.close();
		return PlayerDeatils;
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	try{for(int i=0;i<players.size();i++) {
    		Player temp=players.get(i);
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(int i=0;i<players.size();i++) {
    		Player temp=players.get(i);
    		if((temp.getRunsScored()>criteria[0])&&(temp.getWicketsTaken()>criteria[1])) {
    			allRounder.add(temp);        			
    			}
    		}
    	return allRounder;
