...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		sc.nextLine();//1st line skipped
		while(sc.hasNextLine()){
			String pName= sc.next()+sc.next();
			Role role=Role.fromString(sc.next());
			int runs=sc.nextInt();
			int wikets=sc.nextInt();
			String team=sc.next();
			Player p=new Player(pName,role,runs,wikets,team);
			playersList.add(p);
			sc.nextLine();
		}
		sc.close();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	while(it.hasNext()){
    		...
    		int wikets=p.getWicketsTaken();
    		String team=p.getTeamName();
    		pw.print(name+" "+role+" "+runs+" "+wikets+" "+team+"\n");
    	}
    	pw.close();
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	while(it.hasNext()){
    		...
    		if(p.getPlayerName().equals(playerName)){
    			...
    			int oldWikets=p.getWicketsTaken();
    			p.setWicketsTaken(oldWikets+wickets);
    			break;
    		}
    	}
    	if(flag==false){
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	Iterator<Player> it2=teamPlayers.iterator();
    	double sum=0;
    	while(it2.hasNext()){
    		Player p=it2.next();
    		sum+=p.getRunsScored();
    	}
    	int nop=teamPlayers.size();
    	double average=sum/nop;
    	return average;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	while(it.hasNext()){
    		Player p=it.next();
    		if(p.getRunsScored()>=minRuns && p.getWicketsTaken()>=minWickets){
    			allRounders.add(p);
    		}
    	}
    	return allRounders;
...
public class P2023A7PS0246P_P1 {
