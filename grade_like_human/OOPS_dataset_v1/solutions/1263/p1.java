...
class CricketDataHandler {
		...
		try{
			...
			while(sc.hasNext()){
				...
				StringTokenizer sto=new StringTokenizer(s,",");
					...
					String pn=sto.nextToken();
					Role r=Role.fromString(sto.nextToken());
					int runs=Integer.parseInt(sto.nextToken());
					int w=Integer.parseInt(sto.nextToken());
					String t=sto.nextToken();
				Player p= new Player(pn,r,runs,w,t);
				ls.add(p);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	p.println("PlayerName, Role,RunScored, WicketsTaken,TeamName");
    	for (Player pl: players){
    		String name=pl.getPlayerName();
    		String role=pl.getRole().toString();
    		String runs=String.valueOf(pl.getRunsScored());
    		String w=String.valueOf(pl.getWicketsTaken());
    		String t=String.valueOf(pl.getTeamName());
    		p.println(name+","+role+","+runs+","+w+","+t);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(c!=0){
    		...
    		throw new IllegalArgumentException("No players from the team found");
    	}
...
public class P2022B1A71107P_P1 {
