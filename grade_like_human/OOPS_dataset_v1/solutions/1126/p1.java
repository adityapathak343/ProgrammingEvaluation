...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        // Step 6: Create a new player using this information. [1 mark]
			Player p=new Player(st.nextToken(), (Role)st.nextElement(),(int)st.nextElement(), (int)st.nextElement(),st.nextToken());
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	writer.write("PlayerName,Role,RunsScored,WicketsTake,TeamName\n");
    	...
    	for(Player p:players) {
    		...
    		writer.write(st.nextToken());
    		writer.write((int)st.nextElement());
    		writer.write((int)st.nextElement());
    		writer.write(st.nextToken()+"\n");
    	}
    	writer.close();
...
// Step 4: Similarly, update the player's wickets with the new value. Updated value will be the sum of the old wickets and the argument wickets. [1 mark]    
        		...
        		throw IllegalArgumentException;// Step 5: If no player matches the given name, throw an IllegalArgumentException exception. [1 mark]
    	...
    	try {
    		for(Player p:players) {
    			if(p.getTeamName()==teamName) {
    				totalruns+=p.getRunsScored(); // Step 3: Calculate the total runs scored by all players from this team. [1 mark]
    				playercount++;
    			}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> player=new ArrayList<>();// Step 1: Create an empty list for players matching the criteria. [1 mark]
    	for(Player p:players) {// Step 2: Go through each player in the players list. [1 mark]
			if(p.getTeamName()==teamName) {
				player.add(p); // Step 3: If the player's team matches the given name, add them to the list. [2 marks]
			}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p:players) {// Step 2: Go through each player in the players list. [1 mark]
			if(p.getPlayerName()=="ALL_ROUNDER" && p.getRunsScored()==criteria[0] && p.getWicketsTaken()==criteria[1]) {
				c.add(p);
			}
