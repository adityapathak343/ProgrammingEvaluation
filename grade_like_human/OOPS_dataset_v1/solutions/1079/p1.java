 ...
 * ID Number: 2022B4A70874P
 * Name: Chayan Hitendra Jain
 * Lab Number: 6118
 * System Number: 48
 ***********************************************************/
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while (scanner.hasNextLine()) {
			...
			Player player = new Player(row[0], Role.fromString(row[1]),
					Integer.parseInt(row[2]), Integer.parseInt(row[3]), row[4]);
			players.add(player);
		}
		scanner.close();
	...
	public void writePlayersToFile(String fileName, List<Player> players)
		...
		pw.println("PlayerName,Role,Runs,Wickets,TeamName");
		for (Player player : players) {
			String string = "";
			string = string.concat(player.getPlayerName());
			string = string.concat(",");
			string = string.concat(player.getRole().toString());
			string = string.concat(",");
			string = string.concat(String.valueOf(player.getRunsScored()));
			string = string.concat(",");
			string = string.concat(String.valueOf(player.getWicketsTaken()));
			string = string.concat(",");
			string = string.concat(player.getTeamName());
			pw.println(string);
		}
	...
	public void updatePlayerStats(List<Player> players, String playerName,
		...
		for (Player player : players) {
			if (player.getPlayerName().equals(playerName)) {
				int oldRuns = player.getRunsScored();
				int oldWickets = player.getWicketsTaken();
				player.setRunsScored(oldRuns + runs);
				player.setWicketsTaken(oldWickets + wickets);
				return;
			}
		}
	...
	public double calculateTeamAverageRuns(List<Player> players, String teamName) {
		...
		double av = (double) runs / (double) filteredList.size();
		return av;
	}
}
