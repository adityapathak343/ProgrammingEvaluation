...
//			// Step 5: Split the line into different pieces of information. [1 mark]
			name = file.next();
			full= file.next().split(",");
			name= name+" "+full[0];
			Role = full[1];
			RunScore=Integer.parseInt(full[2]);
			wicketstaken=Integer.parseInt(full[3]);
			teamname=full[4];
//			// Step 6: Create a new player using this information. [1 mark]
			player = new Player(name,null, RunScore, wicketstaken, teamname);
//			// Step 7: Add the new player to the list. [1 mark]
		...
		//type of dealer variable has to be changed to the player in all the code
		return list;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	file.write("PlayerName,Role,RunsScored,WicketsTaken,TeamName\n");
        // Step 3: For each player in the list, convert their details to the desired format. [1 mark]
    	for(int i=0;i<players.size();i++) {
    		file.print(players.get(i).getPlayerName()+","+players.get(i).getRole()+","+players.get(i).getRunsScored()
    				+","+players.get(i).getWicketsTaken()+","+players.get(i).getTeamName()+"\n");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i=0;i<players.size();i++) {
    		...
    		boolean isplayer = playername==playerName?true:false;
    		if(isplayer) {
    			...
    			score=players.get(i).getRunsScored();
    			score+=runs;
    			players.get(i).setRunsScored(score);
    			
    			wicket=players.get(i).getWicketsTaken();
    			wicket+=wickets;
    			players.get(i).setWicketsTaken(wicket);
    		}
    	}
...
public class P2023A7PS0495P_P1 {
