...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputstream.hasNext()){
			...
			String text=inputstream.next();
			String[] parts= text.split(",");
			String name=n1+" "+parts[0];
			String role=parts[1];
			Role r = Role.fromString(role);
			int runs=Integer.parseInt(parts[2]);
			int wickets=Integer.parseInt(parts[3]);
			String team=parts[4];
			players.add(new Player(name, r, runs, wickets, team));
		}
		inputstream.close();
		return players;
    }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		...
    		System.out.println("Problem opening or creating files");
    		System.exit(0);
    	}
    	...
    	for(Player p:players){
    		...
    		Role role=p.getRole();
    		String r=null;
    		if(role==Role.BATSMAN) r="BATSMAN";
    		if(role==Role.BOWLER) r="BOWLER";
    		if(role==Role.ALL_ROUNDER) r="ALL_ROUNDER";
    		int runs=p.getRunsScored();
    		int wickets=p.getWicketsTaken();
    		String team=p.getTeamName();
    		outstream.println(name+","+r+","+runs+","+wickets+","+team);
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player p:players){
    		if(p.getPlayerName().equalsIgnoreCase(playerName)){
    			...
    			int oldw=p.getWicketsTaken();
    			int neww=oldw+wickets;
    			p.setWicketsTaken(neww);
    			c++;
    		}
    	}
    	if(c==0) throw new IllegalArgumentException("Player name does not match with anyone in the list");
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> players2=new TeamFilterStrategy().filter(players, teamName);
    	if (players2.size()==0) throw new IllegalArgumentException("No players from specified team found");
    	int total=0;
    	for(Player p:players2){
    		total = total+p.getRunsScored();
    	}
    	int avg= total/players2.size();
    	return avg;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(Player p:players){
    		if(p.getTeamName().equalsIgnoreCase(teamName)) pl.add(p);
    	}
    	return pl;
    }
}
...
public class P2022B3A70648P_P1 {
