...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		List<String> playerDetail =new ArrayList<>();
		...
		while(inputStream.hasNextLine()){
			...
			playerDetail.add(line);
			StringTokenizer st= new StringTokenizer(line,",");
			List<String> player=new ArrayList<String>();
			while(st.hasMoreTokens()){
				player.add(st.nextToken());
			}
			Role role= Role.fromString(player.get(1));
			Player p=new Player(player.get(0),role, Integer.parseInt(player.get(2)), Integer.parseInt(player.get(3)), player.get(4));
			players.add(p);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	catch(FileNotFoundException e){System.out.println("Error in opening the file");
    	}
    	outStream.println("PlayerName\tRole\tRunsScored\tWicketsTaken\tTeamName");
    	
    	for(Player player: players){
    		outStream.print(player.getPlayerName()+"\t");
    		outStream.print(player.getRole().toString()+"\t");
    		outStream.print(player.getRunsScored()+"\t");
    		outStream.print(player.getWicketsTaken()+"\t");
    		outStream.print(player.getTeamName().toString()+"\t");
    	}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> filtered =ob.filter(players,teamName);
    	if(filtered==null){
    		System.out.println("No player from this team");
    		return 0.0;
    	}
    	else{
    		double sum=0;
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> filtered_list=ArrayList<Player>;
    	for(Player player:players){
    		if(player.getTeamName().equals(teamName)){
    			filtered_list.add(player);
    		}
    	}
