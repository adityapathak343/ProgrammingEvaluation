...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner sc = null;
		try {
			sc = new Scanner(new FileInputStream("src\\inputCricketData.csv"));
		...
		while (sc.hasNextLine()) {
			...
			String[] attrArray = line.split(",");
			String name = attrArray[0];
			Role role = Role.valueOf(attrArray[1]);
			int runs = Integer.parseInt(attrArray[2]);
			int wickets = Integer.parseInt(attrArray[3]);
			String team = attrArray[4];
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for (Player player : players) {
    		...
    		pw.println(name+","+role+","+runs+","+wickets+","+team);
    	}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	throw new IllegalArgumentException("No player with that name was found.");
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if (teamPlayers.isEmpty()) {
    		throw new IllegalArgumentException("No player from tht team was found.");
    	}
    	
    	double mean = (double)total/(double)teamPlayers.size();
    	return mean;
    }
}
...
public class P2022B3A70598P_P1 {
