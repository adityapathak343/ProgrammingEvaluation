...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner pla=null;
		pla=new Scanner(new FileInputStream(fileName));
		...
		int i=0;
		while(pla.hasNextLine()){
			if(i==0){
				continue;
			}
			i++;
			String li=pla.nextLine();
			String[] part=li.split(",");
			list.add(new Player(part[0],Role.fromString(part[1]),Integer.parseInt(part[2]),Integer.parseInt(part[3]),part[4]));
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	writ=new PrintWriter(new FileOutputStream(fileName));
    	...
    	writ.println("PlayerName,Role,RunsScore,WicketsTaken,TeamName");
    	List<Player> list=players;
    	int length=list.size();
    	for(int i=0;i<length;i++){
    		writ.println(list.get(i).toCsvFormat());
    	}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	int length=list.size();
    	for(int i=0;i<length;i++){
    		if(list.get(i).getPlayerName().equals(playerName)){
    			int a=list.get(i).getRunsScored();
    			int b=list.get(i).getWicketsTaken();
    			list.get(i).setRunsScored(a+runs);
    			list.get(i).setWicketsTaken(b+wickets);
    		}
    		else{
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<length;i++){
    		if(list.get(i).getTeamName().equals(teamName)){
    			totalruns=list.get(i).getRunsScored()+totalruns;
    			count++;
    		}
    		else{
    			throw new IllegalArgumentException();
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	int length=players.size();
    	for(int i=0;i<length;i++){
    		if(players.get(i).getRole().fromString("ALL_ROUNDER").equals("ALL_ROUNDER")&&(players.get(i).getRunsScored())==criteria[0]&&(players.get(i).getWicketsTaken())==criteria[1]){
    			list.add(players.get(i));
    		}
    	}
