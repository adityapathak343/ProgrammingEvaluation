...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNext()){
			Player p = new Player(inputStream.next(), inputStream.next(), inputStream.nextInt(), inputStream.nextInt(), inputStream.next())
			list.add(p);
			inputStream.nextLine();
		}
		inputStream.close();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter pt = null;
    	try{
    		pt = new PrintWriter(new FileOutputStream(fileName));
    	} catch(FileNotFoundException e){
    		e.printStackTrace();
    	}
    	pt.print("PlayerName");
    	pt.print("Role");
    	pt.print("RunsScored");
    	pt.print("WicketTaken");
    	pt.print("TeamName");
    	pt.print("\n");
    	for(int i = 0;i < players.size();i++){
    		pt.print(players.get(i).getPlayerName());
        	pt.print(players.get(i).getRole());
        	pt.print(players.get(i).getRunsScored());
        	pt.print(players.get(i).getWicketsTaken());
        	pt.print(players.get(i).getTeamName());
        	pt.print("\n");
    	}
    	pt.close();
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i = 0;i<players.size();i++){
    		if(players.get(i).getPlayerName() == playerName){
    			int totalRuns = players.get(i).getRunsScored() + runs;
    			players.get(i).setRunsScored(totalRuns);
    			int totalWickets = players.get(i).getWicketsTaken() + wickets;
    			players.get(i).setWicketsTaken(totalWickets);
    		}
    		else{
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	ArrayList sameTeamList = new ArrayList<Player>();
    		for(int i = 0;i<players.size();i++){
    			if(players.get(i).getTeamName() == teamName){
    				sameTeamList.add(players.get(i));
    			}
    			else{
    				throw (IllegalArgumentException);
    			}
    		...
    		int totalRuns = 0;
    		for(int i = 0;i<sameTeamList.size();i++){
    			totalRuns += sameTeamList.get(i).getRuns();
    		}
    		double averageRuns = totalRuns/sameTeamList.size();
    		return averageRuns;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	ArrayList filterPlayer = new ArrayList<Player>();
    	for(int i = 0;i<players.size();i++){
    		if(players.get(i).getTeamName() == teamName){
				filterPlayer.add(players.get(i));
			}
    	}
    	return filterPlayer;
    }
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	ArrayList allRounderList = new ArrayList<Player>();
    	for(int i = 0;i<players.size();i++){
    		if(players.get(i).getRole() == "ALL_ROUNDER" && players.get(i).getRunsScored() >= criteria[0] && players.get(i).getWicketsTaken() >= criteria[1] ){
    			allRounderList.add(players.get(i));
    		}
    	}
