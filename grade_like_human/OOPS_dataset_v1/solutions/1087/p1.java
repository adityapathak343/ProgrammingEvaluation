...
//       }
        ...
        // Step 2: Open the specified file for reading data. [1 mark]
    	   inputstream.skip("PlayerName	Role	RunsScored	WicketsTaken	TeamName ");
          while ( inputstream.hasNextLine()){ // Step 3: Ignore the first line since it contains the column names. [1 mark]
        ...
        // Step 7: Add the new player to the list. [1 mark]
          Player p1 = new Player (values[0] , Role.valueOf(values[1]) , Integer.parseInt(values[2]) , Integer.parseInt(values[3]) , values[4]);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	outputstream = new PrintWriter ( new FileOutputStream("inputCricketData.csv"));
    	outputstream.println(" PlayerName" + "RunsScored"  + "WicketsTaken" + "TeamName");
    	for ( Player p : players){
    	String name = p.getPlayerName();
    	Role r = (Role) p.getRole();
    	Integer a = (Integer) p.getRunsScored();
    	Integer c = (Integer) p.getWicketsTaken();
    	String Teamname = p.getTeamName();
    		outputstream.println(name + r + a + c + Teamname);
    	}
...
public class P2022B3A70631P_P1 {
