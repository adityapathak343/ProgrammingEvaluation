...
* ID Number:2023A7PS0634P
* Name:NAKSHATARA GARG
* Lab Number:6013
* System Number:14
***********************************************************/
...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	if(p1.runsScored==p2.runsScored && p1.runsScored>p2.runsScored){
    		return 1;
    	}else(p1.runsScored<p2.runsScored){
    		return -1;
    	}
    }
}
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		List<Player> details= new List<>();
		try{
			FileInputStream File=new FileInputStream("inputCricketData.csv");
			int i=1
					while((i=File.read())!=0){
						System.out.print((char)i);
					}
			details.add("viratkohli", "BAtsman","5000","0","RCB");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		Printwriter outstream= new Printwriter(new FileOutputStream("inputCricketData.csv"));
    		
    	}catch(FileNotFoundException e){
    		System.err.println("Error in opening the file");
    		System.exit(0);
    	}
    	outstream.print("Player Name");
    	oustream.print("Role");
    	outstream.print("RunsScored");
    	outstream.print("Wickets taken");
    	outstream.print("TeamName");
    	for(int i=1;i<=21;i++){
    		outsream.println(i)
    	}
    	outstream.close();
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i=0;i<=players.size();i++){
    		players(i);
    		if(this.playerName==players.playerName(i)){
    			this.runs=this.runs+players.runs(i);
    			this.wickets=this.wickets+players.wickets(i);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(players.name(i)==players.teamName(i)){
    		for(int i=0;i<=player.name(i).teamName(i).size();i++){
    		runsScored=player.runscored(i).name(i).teamName(i);
    		int AverageRuns= runsScored / players.name(i).teamName(i).size();
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<=players.size();i++){
    		players(i);
    		if(players.teamName==this.teamName){
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	For(int i=0; i<=players.size(); i++){
    		players(i);
    		if(players.criteria[0]==this.criteria[0] && players.criteria[1]==this.criteria[1]){
    			players.add(this.players);
    			System.out.println("The Player is Good Allrounder and Selected for team");
    		}
    	}
    	return players;
