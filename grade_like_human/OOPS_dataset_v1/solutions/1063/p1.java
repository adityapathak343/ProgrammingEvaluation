...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while((playerDetails=br.readLine())!=null) {
			String[] details=playerDetails.split(",");
			Role role= Role.ALL_ROUNDER;
			if(details[1]=="BOWLER") {
				role = Role.BOWLER;
			}
			if(details[1]=="BATSMAN") {
				role = Role.BATSMAN;
			}
			if(details[1]=="ALL_ROUNDER") {
				role = Role.ALL_ROUNDER;
			}
			
			Player temp = new Player(details[0],role,Integer.parseInt(details[2]),Integer.parseInt(details[3]),details[4]);
			players.add(temp);
		}
		br.close();
		return players;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	String first = String.format("%s,%s,%d,%d,%s",
    			"PlayerName", "Role", "RunsScored", "WicketsTaken", "TeamName");
    	pw.println(first);
    	for(int i=0;i<players.size();i++) {
    		String player=players.get(i).toCsvFormat();
    		pw.println(player);
    	}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i=0;i<players.size();i++) {
    		if(players.get(i).getPlayerName()==playerName) {
    			...
    			players.remove(i);
    			temp.setRunsScored(temp.getRunsScored()+runs);
    			temp.setWicketsTaken(temp.getWicketsTaken()+wickets);
    			players.add(temp);
    			break;
    		}
    		else {
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<players.size();i++) {
    		if(players.get(i).getTeamName()==teamName) {
    			count++;
    			runs+=players.get(i).getRunsScored();
    		}
    	}
    	if(count!=0) {
    		double result = (double) runs/count;
    		return result;
    	}
    	else {
    		throw new IllegalArgumentException();
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	ArrayList<Player> filtered = new ArrayList<>();
    	for(int i=0;i<players.size();i++) {
    		if(players.get(i).getRole().toString()=="ALL_ROUNDER"){
    			if(players.get(i).getRunsScored()>=criteria[0]&&players.get(i).getRunsScored()>=criteria[0]) {
        			filtered.add(players.get(i));
        		}
    		}
    	}
...
public class P2022B3A70495P_P1{
