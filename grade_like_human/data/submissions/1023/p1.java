...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
        char[] players;// Step 1: Create an empty list to store player details. [1 mark]
		File f1=new File("C:\Users\user\Desktop\SS_24_25_CBT");// Step 2: Open the specified file for reading data. [1 mark]
        // Step 3: Ignore the first line since it contains the column names. [1 mark]
        char read(char PlayerName[])// Step 4: Read each line one by one until reaching the end of the file. [1 mark]
        ...
        // Step 9: Return the complete list of players. [1 mark]
			...
			System.out.println("this part has some errors");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
        FileInputStream f0=new FileInputStream("C:\Users\user\Desktop\SS_24_25_CBT");// Step 1: Prepare to write data into the specified file. [1 mark]
        for ( int i=0;i<buf.length;i+=2) f0.write(buf[i])// Step 2: Write the column names as the first line of the file. [1 mark]
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	{
    		System.out.prinyln("have error in IllegalArgumentException");
    	}
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
        ...
        if(teamName==India){
        	...
        	char[]teammembers=playerNames;// Question 6: Write code for filtering players by team [Total: 5 marks]
        	for(i=0;i<12;i++){
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        if(role==allrounders){
        	char[]
        	System.out.println("")// Question 7: Write code for filtering all-rounders by stats [Total: 5 marks]
        }
