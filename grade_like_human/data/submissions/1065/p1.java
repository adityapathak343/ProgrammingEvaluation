...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	for(int i=0;i<Player.length;i++){
    		if(Player[i].getRunsScored()<Player[i+1].getRunsScored()){
    			Player[i]=int score_i;
    			Player[i]=Player[i+1].getRunsScored();
    			Player[i+1]=score_i;
...
class CricketDataHandler {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		outStream=new PrintWriter(new FileOutputStream("filrname.txt"));
    		for(int i=0;i<Player.length;i++){
    			outStream.println(Player[i]);
    		}
    	}
    	catch(IOException e){
    		System.err.println(e.Player());
    	}
    	finally{
    		outputStream.close();
    	}   
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<Player.length;i++){
    		int total_runs=0;
    		total_runs+=Player.runScored(i);
    	}
    	int avg=total_runs/Playere.length;
    	System.out.println(avg);
    	for(int i=0;i<Player.length;i++){
    		if (Player[i].teamName="India"){
    			Team[] Team1= new Team[i];
