...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner ipstr=null;
		try {
			ipstr=new Scanner(new FileInputStream("C:/Users/FDUSER/eclipse-workspace2/CBT/inputCricketData.csv"));
		...
		while(ipstr.hasNextLine()) {
			...
			String name=spt[0];
			Role role=Role.valueOf(spt[1]);
			int runs=Integer.valueOf(spt[2]);
			int wickets=Integer.valueOf(spt[3]);
			String team=spt[4];
			// Step 6: Create a new player using this information. [1 mark]
			Player plr=new Player(name,role,runs,wickets,team);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter opstr=null;
    	try {
    		opstr=new PrintWriter(new FileOutputStream(fileName));
    	...
    	opstr.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
        // Step 3: For each player in the list, convert their details to the desired format. [1 mark]
    	for(Player plr: players) {
    		String line=plr.toCsvFormat();
//    		System.out.println(line);
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player plr: players) {
    		...
    		if(plr.getPlayerName().equals(playerName)) {
    			flag++;
    			...
    			plr.setRunsScored(plr.getRunsScored()+runs);
...
//    	if(flag==0) throw(IllegalArgumentException e); 
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(Player plr: players) {
    		if(plr.getTeamName().equals(teamName)) {
    			count++;
    			teamList.add(plr);
    		}
    	}
...
//    	if(count==0) throw(IllegalArgumentException);
    	...
    	for(Player plr: teamList) {
    		runSum+=plr.getRunsScored();
    	}
        // Step 4: Compute and return the average runs scored. [1 mark]
    	double avg=(double)runSum/count;
    	return avg;
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player plr:players) {
    		...
    		if(plr.getRole().equals(Role.ALL_ROUNDER)&&plr.getRunsScored()>=criteria[0]&&plr.getWicketsTaken()>=criteria[1]) {
    			pList.add(plr);
    		}
    	}
        // Step 4: Return the list containing all matching players. [1 mark]
...
public class F2023A7PS0677P_P1 {
