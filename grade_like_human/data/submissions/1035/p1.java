...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        String lineIgnore = inputStream.nextLine();
        ...
        while(inputStream.hasNextLine()) {
        	...
        	Player playerCricket = new Player(arrLine[0], Role.valueOf(arrLine[1]), Integer.parseInt(arrLine[2]), Integer.parseInt(arrLine[3]), arrLine[4]);
        	// Step 7: Add the new player to the list. [1 mark]
        	playerDetails.add(playerCricket);
        } //end of the loop        
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        for (int i = 0; i < players.size(); i++) {
        	outStream.println(players.get(i).getPlayerName() + "," + (players.get(i).getRole()) + "," + players.get(i).getRunsScored() + "," + players.get(i).getWicketsTaken() + "," + players.get(i).getTeamName());
        }
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	try {
	    	for (int j = 0; j < players.size(); j++) {
	    		Player checkPlayer =  players.get(j);
	    		// Step 2: Check if the current player's name matches the given name. [1 mark]
	    		if (checkPlayer.getPlayerName() == playerName) {
	    			...
	    			checkPlayer.setRunsScored(runs + checkPlayer.getRunsScored());
	    			...
	    			checkPlayer.setWicketsTaken(wickets + checkPlayer.getWicketsTaken());
	    		}
	    	}
    	}
    		// Step 5: If no player matches the given name, throw an IllegalArgumentException exception. [1 mark]
    	catch (IllegalArgumentException e) {
    		System.err.println("Error illegal argument exception " + e.getMessage());
    	}
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	try {
	    	...
	    	for (int k = 0; k < players.size(); k++) {
	    		if (players.get(k).getTeamName() == teamName) {
	    			filteredTeams.add(players.get(k));
	    			total = total + players.get(k).getRunsScored();
	    			filteredNo += 1;
	    		}
	    	}
	    	average = 1.0 * total /filteredNo;
	    }
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for (int m = 0; m <players.size(); m++) {
    		if (players.get(m).getTeamName() == teamName) {
    			filter.add(players.get(m));
    		}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	ArrayList allRounder = new ArrayList();
        // Step 2: Go through each player in the list. [1 mark]
        for (int n = 0; n < players.size();  n++) {
        	if (players.get(n).getRunsScored() >= criteria[0] && players.get(n).getWicketsTaken() >= criteria[1]){
        		allRounder.add(players.get(n));
        	}
        }
...
public class P2022B3A70212P_P1 {
