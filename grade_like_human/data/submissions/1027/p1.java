	...
	***********************************************************/
	...
	class CricketDataHandler {
		...
		public List<Player> readPlayersFromFile(String fileName) throws IOException {
			...
			List <Player> pDetails = new ArrayList();
	    	Scanner inputStream = null;
	    	 try (Scanner inputStream1 = new Scanner(new File(fileName))) { 
	    ...
	    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
	    	...
	    	outputStream = new PrintWriter(new FileOutputStream("PlayerData"));
	    	outputStream.println("PlayerName	Role	RunsScored	WricketsTaken	TeamName");
	    ...
	    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
	    	...
	    	for(int i = 0 ; i<players.size(); i++){
	    		if(((Player) players).getPlayerName()== playerName){
	    			count++;
	    			((Player) players).setPlayerName(playerName);
	    			int a = ((Player) players).getRunsScored();
	    			((Player) players).setRunsScored(runs + a);
	    			
	    			
	    			int b = ((Player) players).getWicketsTaken();
	    			((Player) players).setWicketsTaken(wickets + b);
	...
	class TeamFilterStrategy implements PlayerFilter<String> {
	    ...
	    public List<Player> filter(List<Player> players, String teamName) {
	    	...
	    	int count =0;
	    	for(int i = 0 ; i<players.size(); i++){
	    		if(((Player) players).getTeamName()== teamName){
	    			count++;
	    			filterPlayer.addAll(players);
	...
	class AllRounderStatsFilter implements PlayerFilter<int[]> {
	    ...
	    public List<Player> filter(List<Player> players, int[] criteria) {
	    	...
	    	int count =0;
	    	for(int i = 0 ; i<players.size(); i++){
	    		if(((Player) players).getRole()== Role.ALL_ROUNDER){
	    			if(((Player) players).getRunsScored()== 256){
		    			if(((Player) players).getWicketsTaken()== 18){
		    				count++;
		    				allRounders.addAll(players);
	    			}
	    		}			
	    		}
	    		if(count == 0){
