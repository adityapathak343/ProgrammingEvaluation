...
import HarshGoswami_LabTest.BufferedReader;
import HarshGoswami_LabTest.FileReader;
import HarshGoswami_LabTest.IOException;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			...
			i = new Scanner(new FileInputStream("fileName"));
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
		...
		for(Player p :players){
    		...
    		if(p.getPlayerName()== playerName){
    			int i = p.getRunsScored();
    			i += runs;
    			p.setRunsScored(i);
    			
    			int k = p.getWicketsTaken();
    			k += wickets;
    			p.setWicketsTaken(k);
...
public class P2022A3PS1217P_P1 {
