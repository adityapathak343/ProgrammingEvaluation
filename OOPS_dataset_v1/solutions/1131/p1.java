...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		private List<Player> ls = new ArrayList<Player>();
        // Step 2: Open the specified file for reading data. [1 mark]
		try{
			File f = new File("inputCricketData.csv");
		}
...
//		for(int j=0;j<i;j++){
//		StringTokenizer st = new StringTokenizer(data[j],"");
		String playerName = scan.next();
		Role role = fromString(scan.next());
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p : players){
    		Role role = new fromString("ALL_ROUNDER");
    		if(p.getRole()== role){
    			int runs = 100000000;
    			int wickets = 100000000;
    			if(runs>p.getRunsScored()){
    				runs = p.getRunsScored();
    				p1 = p;
    			}
    			if(wickets>p.getWicketsTaken()){
