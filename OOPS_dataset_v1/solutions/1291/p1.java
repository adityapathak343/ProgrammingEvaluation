...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
			...
			Player newplayer= new Player("Ansh", Role.ALL_ROUNDER ,1200 ,100,"India");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
    		...
    		outstream.println("player name");
    		outstream.println("role");
    		outstream.println("run scored");
    		outstream.println("wickets taken");
    		outstream.println("team name");
    		for(Player a : players) {
    			outstream.println(a.getPlayerName());
    			outstream.println(a.getRole());
    			outstream.println(a.getRunsScored());
    			outstream.println(a.getRunsScored());
    			outstream.println(a.getWicketsTaken());
    		}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player b : players) {
    		try {
    		if(b.getPlayerName()==playerName) {
    			b.setRunsScored(runs + b.getRunsScored());
    			b.setWicketsTaken(wickets + b.getWicketsTaken());
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(Player c : players) {
			...
			double z;
    		try {
    			if(c.getTeamName()==teamName) {
    				totalruns += c.getRunsScored();
    				z++;
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> addplayer = new ArrayList<>();
