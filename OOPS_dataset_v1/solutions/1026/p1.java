...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try { 
			...
			while(reader.hasNext()) {
				String playerInfoString = reader.nextLine();
				String[] infoStrings = playerInfoString.split(",");
				players.add(new Player(infoStrings[0], Role.fromString(infoStrings[1]), new Integer(infoStrings[2]).intValue(),new Integer(infoStrings[3]).intValue(), infoStrings[4]));
			...
			System.out.println("Exception some other " + e.getMessage());
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try { 
			...
			writer.print("Roles, ");
			writer.print("RunsScored, ");
			writer.print("WicketsTaken, ");
			writer.print("TeamName, ");
			Iterator itr = players.iterator();
			while(itr.hasNext()) {
				Player p = (Player)itr.next();
				writer.println(p.getPlayerName() +",");
				writer.print(p.getRole()+",");
				writer.print(p.getRunsScored() +",");
				writer.print(p.getWicketsTaken() +",");
				writer.print(p.getTeamName() +",");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	throw new IllegalArgumentException("Player name doesnt match"); 
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	while(itr.hasNext()){
    		...
    		sum = sum + p.getRunsScored();
    		noOfPlayers++;
    		}
    	}
    	
    	if (noOfPlayers != 0) {return (sum/noOfPlayers);}
    	else {throw new IllegalArgumentException("No players from this team");}
    }
...
public class P2023A7PS0570P_P1 {
    ...
    public static void main(String[] args) {
        ...
        try {
            // Read data from file
            players = dataHandler.readPlayersFromFile("C:\\Users\\user\\Desktop\\inputCricketData.csv");
        } catch (FileNotFoundException e) {
