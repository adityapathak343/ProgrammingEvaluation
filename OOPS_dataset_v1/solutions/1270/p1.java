    	...
    	for(Player p : players) {
    		try {
    			if(p.getPlayerName()==playerName) {
    				int n=p.getRunsScored();
    				p.setRunsScored(n+runs);
    				int m=p.getWicketsTaken();
    				p.setWicketsTaken(m+wickets);
    			}
    		}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	ArrayList<Player> list=new ArrayList<>();
    	Role r = Role.ALL_ROUNDER;
    	for(Player p : players) {
    		if(p.getRole()==r && p.getRunsScored()>=criteria[0] && p.getWicketsTaken()>=criteria[1]) {
    			list.add(p);
    		}
    	}
    	return list;
...
public class F2022B4A70627P_P1 {
