...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			inputstr = new Scanner(new FileInputStream("/CBT/src/inputCricketData.csv"));
			inputstr.nextLine();
			while(inputstr.hasNext()){
				
			String name = inputstr.next();
			String roles = inputstr.next();
			int runs = inputstr.nextInt();
			int wickets = inputstr.nextInt();
			String team = inputstr.next();
			Role rolec = Role.fromString(roles);
			Player curr_player = new Player(name, rolec,runs, wickets, team);
			playerlist.add(curr_player);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		outputstr = new PrintWriter(new FileOutputStream(fileName));
    		outputstr.println("NAME" + "\'" + "ROLE" + "\'" + "RUNS" +"\'"+ "WICKETS" +"\'"+"TEAM");
    		for (Player p : players){
    			outputstr.println(p.toCsvFormat());
    		}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	return ((teamcnt>0)?totalscore/teamcnt:0.0);
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	ArrayList<Player> sameTeam = new ArrayList<Player>();
    	for (Player p : players){
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	ArrayList<Player> match_criteria = new ArrayList<Player>();
    	
    	for( Player p : players){
    		if(p.getRole().equals(Role.ALL_ROUNDER)){
    			if((p.getRunsScored() >= criteria[0]) && (p.getWicketsTaken()>=criteria[1])){
    				match_criteria.add(p);
    			}
    		}
    	}
...
public class P2023A7PS0550P_P1 {
