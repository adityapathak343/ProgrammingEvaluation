...
//    } 	List<Player> player = new ArrayList<>();
    	Scanner in=null;
    	try{
    		in = new PrintWriter(new FileOutputStream("inputcricketData"));
    		...
    		while(in.hasNextLine()){
    			...
    			player.outstream(playerName,role,runsScored,wicketsTaken,teamName);
    		}
...
//    	try{
//    		outstream = new PrintWriter(new FileOutputStream("inputcricketData"));
//    		while (in.hasNextLine()){
//    			String line = in.nextLine();
//    			System.out.println(in.next());
//    		}
...
//    			String playerName =in.nextLine();
//    			String role = in.nextLine();
//    			Double runsScored = in.nextDouble();
//    			Double wicketsTaken = in.nextDouble();
//    			String teamName = in.nextLine();
//    			player.outstream(playerName,role,runsScored,wicketsTaken,teamName);
//    			
//    			
...
//    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	Iterator<Player>  i = new  players.iterator();
    	while(i.hasNext()){
    		try{
    			if(i.getPlayerName.equals(playerName))
    			{
    				i.runsScored =runs;
    				i.wicketsTaken = wickets;
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	return total_runs/no_of_player;
...
//    	List<Player> Playerbyrun = new ArrayList<>();
    	List<Player> allrounders = new ArrayList<>();
    	Iterator<Player>  i = new  players.iterator();
    	while(i.hasNext()){
    		if(i.role.equals("ALL_ROUNDER")&&i.getRunsScored>=criteria[0]&&i.getWicketstaken>=criteria[1]);
    		{
    			allrounders.add(i);
    		}
    		return allrounders;
...
public class F2023A7PS05558P_P1 {
