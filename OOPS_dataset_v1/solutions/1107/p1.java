...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	if(player1.getRunsScored()>player2.getRunsScored()){
    		return -1;
    	}
    	else if(player1.getRunsScored()<player2.getRunsScored()){
    		return 1;
    	}
    	else{
    		return 0;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			...
			while(sc.hasNext()){
				...
				Role role= (Role)Role.fromString(info[1]);
				int runsScored= Integer.parseInt(info[2]);
				int wicketsTaken= Integer.parseInt(info[3]);
				String teamName= info[4];
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		...
    		p.println("PlayerName, Role, RunsScored, WicketsTaken, TeamName");
    		for(int i=0;i<players.size();i++){
    			//String line= "";
        		Player player= players.get(i);
        		String line=player.getPlayerName()+"," +player.getRole()+","+player.getRunsScored()+","+player.getWicketsTaken()+","+player.getTeamName();
        		p.println(line);
    		}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if(tplayers.size()==0){
    		throw new IllegalArgumentException("No player could be found in this team");
    	}
    	double sum=0, avg=0;
    	for(int i=0;i<tplayers.size();i++){
    		sum+= tplayers.get(i).getRunsScored();
    	}
    	avg= sum/tplayers.size();
    	return avg;
    }
}
...
public class F2023A7PS0006P_P1 {
