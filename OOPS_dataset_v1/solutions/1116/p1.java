...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try {
           ...
           reader=new Scanner(new FileInputStream("\"C:\\Users\\user\\Downloads\\SS_24_25_CBT\\SS_24_25_CBT\\inputCricketData.csv\""));
		}
		catch(FileNotFoundException e){
			System.out.println("file does not find in  the system");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
    		output=new PrintWriter(new FileOutputStream("\"C:\\Users\\user\\Downloads\\SS_24_25_CBT\\SS_24_25_CBT\\outputCricketData.csv\""));
    	}
    	catch(FileNotFoundException e) {
    		System.out.println("the file does not found in computer");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	public boolean equals(Object obj){
    		if(players.playerName==obj) return true;
    		if(obj==null|| getclass()!=obj.getclass()) return false;
    			
    		players player= (player)obj;
    	}
    return players.run+ new players.run;
    }
    ...
    try {
    	String playerNmae;
    	if(playerName.players!= playerNmae.team) {
    		
    	}
    }
    catch(IllegalArgumentException e){
    	System.out.println("player doesnot belong to the team");
    	
    }
    int totalrun=0;
    for(int i =0; i<12; i++) {
    	int  totalrun=totalrun+playerName.run;
    }
   // float average;
     float average=(totalrun)/team.players;
     return average;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(int i=0; i<12; i++) {
    		Iterator<String> itr= players.itr();
    		while(itr.hasnext()) {
...
public class P2023A7PS0522P_P1 {
