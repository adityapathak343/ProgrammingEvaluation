...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	Player pf = (Player)p1;
    	Player ps = (Player)p2;
        if(pf.getRunsScored()< ps.getRunsScored())
        	return 1;
        if(pf.getRunsScored() > ps.getRunsScored())
        	return -1;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		add.Player("New Player", "Role",0, 0, "Team");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	try{
    		if(getplayerName == playerName){
    			this.getRunsScored = runs + runsScored;
    		}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	List<Player> TeamAverage = new ArrayList<>();
    	List<Player> TotalTeamMemebrs = new ArrayList<>();
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	filter(List<Players> players, String teamName){
    	...
    	ArrayList IndiaTeam = new ArrayList();
    	ArrayList AustraliaTeam = new ArrayList();
    	ArrayList EnglandTeam = new ArrayList();
        // Step 2: Go through each player in the players list. [1 mark]
    	for(int i =0; i< 22;i++){
    		if(teamName == "India"){
    			IndiaTeam.add(this.playerName);
    		}
    		else if(teamName == "Australia"){
    			AustraliaTeam.add(this.playerName);
    		}
    		else{
    			EnglandTeam.add(this.playerName);
    		}
    	}
