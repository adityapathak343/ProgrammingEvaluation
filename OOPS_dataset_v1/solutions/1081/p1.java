...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(s.hasNextLine()){
			...
			Integer i2=new Integer(sa[2]);
			Integer i3=new Integer(sa[3]);
			Role r1=Role.fromString(sa[1]);
			Player p1=new Player(sa[0],r1,i2,i3,sa[4]);
			l.add(p1);
		}
		s.close();
		return l;
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> teamPlayer= t.filter(players, teamName);
    	//if(teamPlayer.size()==0) throw new IllegalArgumentException();
    	double sum=0.0;
    	int noOfP=0;
...
public class F2022B3A71091P_P1 {
