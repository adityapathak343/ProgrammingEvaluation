...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(in.hasNextLine()){
			...
			Player p1= new Player(data[0], Role.valueOf(data[1]), Integer.parseInt(data[2]), Integer.parseInt(data[3]), data[4]);
			list.add(p1);
		}
		in.close();
		return list;
...
//    		pw.println(pp.getPlayerName()+","+pp.);
    		String requiredFormat=pp.toCsvFormat();
    		pw.println(requiredFormat);
    	}
    	pw.close();
    }
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player pp: players){
    		if(pp.getPlayerName().equals(playerName)){
    			present=1;
    			pp.setRunsScored(runs+pp.getRunsScored());
    			pp.setWicketsTaken(wickets+pp.getWicketsTaken());
    		}
    	}
//    	Exception IllegalArguementException=new Exception();
    	if(present==0) throw new IllegalArgumentException();
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	int total=0;
    	int totalRuns=0;
    	List<Player> given= new ArrayList<Player>();
    	for(Player pp: players){
    		if(pp.getTeamName().equals(teamName)){
    			given.add(pp);
    			total++;
    		}
    	}
    	if(total==0){
    		...
    		for(Player pp: given){
    			totalRuns+=(pp.getRunsScored());
    		}
    	}
    	double div=total;
    	return (totalRuns/div);
    }
}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> match =new ArrayList<>();
    	for(Player pp: players){
    		if(pp.getRole().toString().equals("ALL_ROUNDER")){
    			if(pp.getRunsScored()>=criteria[0]&&pp.getWicketsTaken()>=criteria[1]){
    				match.add(pp);
    			}
    		}
    	}
...
public class P2023A7PS0628P_P1 {
