...
class CricketDataHandler {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for(Player p : players){
    		String s = "";
    		s = s + p.getPlayerName();
    		s = s + " "+String.valueOf(p.getRole());
    		s = s + " "+ String.valueOf(p.getRunsScored());
    		s = s + " "+ String.valueOf(p.getWicketsTaken());
    		s = s + " "+ p.getTeamName();
    		output.println(s);
    	}
    	output.close();
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> l = new ArrayList<>();
    	for(Player p : players){
    		String s = String.valueOf(p.getRole());
			if(s.equalsIgnoreCase("AllRounder")){
				if(p.getRunsScored()>=criteria[0] && p.getWicketsTaken()>=criteria[1]) l.add(p);
			}
		}
    	return l;
    }
