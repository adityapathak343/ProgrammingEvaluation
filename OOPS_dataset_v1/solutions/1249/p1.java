...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
        ...
        int p1_runs = p1.getRunsScored();
        int p2_runs = p2.getRunsScored();
        return (p2_runs - p1_runs);
    }
}
...
class CricketDataHandler {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        buffer_list.add(0,"PlayerName,Role,RunsScored,WicketsTaken,TeamName");
        for(Player line: players) {

            String line_mod = line.toCsvFormat();
            buffer_list.add(line_mod);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
        ArrayList<Player> p_arr = new ArrayList<>();
        try{
            for(Player p : players) {
                String j = p.getTeamName();
                if(j.equals(teamName)) {
                    p_arr.add(p);
                }
            }
            Double sum = 0.0;
