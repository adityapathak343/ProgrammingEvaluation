    ...
    public int compare(Player p1, Player p2) {
        ...
        // a poprivate 
    	 private String playerName;
    	    ...
    	    private int wicketsTaken;
    	    private String teamName;
    	return Double.compare(this.p1, this.p2);
    		Collections.sort(runsScored) ;
        	if ( p1> p2) {return -1;}
        	 if{ p1<p2){return 1;}
             if(p1=p2) {return 0;}    	     
        	 }
...
//		int player=1;
//		while(line != null) { //reads next line that is player
//			if(player>1) {
...
//		}
		...
		int n=sc.nextInt();
		sc.nextLine();
		List<String,Role,Integer,Integer,String>> list=new ArrayList<>();
	     for(int i=0;i<n;i++) {
	    	 String line=sc.nextLine();
	    	 String[] parts=line.split("");
	    	 String colName=parts[0];
	    	 int score=Integer.parseInt(parts[1]);
	    	 colName.put(playerName,runsScored, wicketsTaken, teamName, teamName);
	    	 sc.close();
	    	 List< String,Role,Integer,Integer,String> sco =new ArrayList<>();
	    
	    	 
	    	 //ALL THE STEPS DONE TOGETHER
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	Iterator <Entry<Integer,Integer>> iterator=((Object) players).EntrySet().iterator;
    			try {
					while (Iterator.hasNext()) {
						Entry<Integer,Integer> entry=iterator.next();
								System.out.println ("entry.setKey()");
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	List<Player,String> li=new ArrayList<>();
    }
