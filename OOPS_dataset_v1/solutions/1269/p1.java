...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(myscanner.hasNext()) {
			...
			Role role = Role.fromString(tokens[1]);
			int runs = Integer.parseInt(tokens[2]);
			int wickets = Integer.parseInt(tokens[3]);
			String team = tokens[4];
			Player temp_player = new Player(name, role, runs, wickets, team);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	System.out.println("writing players to file");
    	...
    	printer.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
    	for (Player p : players) {
    		printer.println(p.getPlayerName() + "," + p.getRole() + "," + p.getRunsScored() + "," + p.getWicketsTaken() + "," + p.getTeamName());
    	}
    	System.out.println("Written players!");
    	printer.close();
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	if (!found) {
    		throw new IllegalArgumentException("Player with teamname: " + teamName + " not found!");
    	}
    	
    	return (double)runs/total;
    }
}
...
public class P2022A8PS1260P_P1 {
