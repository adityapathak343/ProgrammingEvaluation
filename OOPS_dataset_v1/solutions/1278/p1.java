...
class CricketDataHandler {
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	double totalRunsinteam = 0;
    	double totalPlayersinteam = 0;
    	for (Player player : players){
    		if (player.getTeamName()== teamName){
    			totalRunsinteam = totalRunsinteam + player.getRunsScored();
    			totalPlayersinteam ++;
    		}
    		
    	}
    	double average = totalRunsinteam/totalPlayersinteam;
    	return average;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	List<Player> playercriteriaList = new ArrayList<>();
    	for (Player player : players){
    		if (player.getTeamName()== teamName){
    			 playercriteriaList.add(player)
    		}
    		return playercriteriaList;
    	}
