...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	return Integer.compare(s1.getRunsScored(), s2.getRunsScored())
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		List<Player> detail = new ArrayList<>();
		List<String> spli = new ArrayList<>();
		try (BufferReader br = new BufferReader(new FileReader(filename))){
			...
			while((line = br.readline()) != null) {
				for (i : line) {
					spli = line.split(,);
					Player x = new Player();
					x.setPlayerName(spli[0]);
					x.setRole(spli[1]);
					x.setRunsScored(Integer.parseInt(spli[2]));
					x.setTeamName(spli[4]);
					x.setWicketsTaken(Integer.parseInt(spli[3]));
					x.set
				}
				detail.add(Player x);
			}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try(PrintWriter= new PrintWriter(new FileWriter(filename))){
    		write.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName")
    		for(x:detail) {
    			write.println(x.getPlayerName()+","+x.getRole()+","+x.getRunScored()+","x.getWicketsTaken()+","+x.getTeamName());
    		}
    	}
    	catch(IOexception e) {}
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
        ...
        // Step 1: Go through each player in tche list. [1 mark]
        // Step 2: Check if the current player's name mathes the given name. [1 mark]
    	...
    	for(i:players) {
    		if(i.getPlayerName() == i.getPlayerName()) {
    			i.setRunsScored(i.getRunsScored + runs);
    			i.setWicketsTaken(i.getWicketsTaken + runs);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> england = new ArrayList<>();
    	int eng=0;
    	double av = 0;
    	int count1 = 0;
    	List<Player> australia = new ArrayList<>();
    	int aus = 0;
    	int count2 = 0;
    	double ausav = 0 ;
    	List<Player> india = new ArrayList<>();
    	int ind = 0;
    	double indav = 0;
    	int count3 = 0;
    	for (i:players) {
    		if(i.getTeamName() == "England") {
    			england.add(Player i);
    			eng = eng + i.getRunsScored();
    			count1 +=1;
    			
    		}
    		else if(i.getTeamName() == "Australia") {
    			australia.add(Player i);	
    			aus = aus + i.getRunsScored();
    			count2 +=1;
    		}
    		else if(i.getTeamName() == "India") {
        		india.add(Player i);
        		ind= ind + i.getRunsScored();
        		count3 +=1;
        	}
    		else {
    			throw (IllegalArgumentException e);
    	...
    	return av = (eng + ind + aus) /(count1 + count2 + count3) ;
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	// I ACCEDENTLY DID THIS FOR THE PREVIOUS QUESTION AND MERGEDTHESE 2 TOGETHER SO I AM PASTING THE SAME CODE
    	// PLS CHECK IT AND donot CROSS IT OUT THANK YOU
    	List<Player> england = new ArrayList<>();
    	int eng=0;
    	double av = 0;
    	int count1 = 0;
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for (i:players) {
    		if(i.getRole() == "ALL_ROUNDER") {
    			if(i.getWicketsTaken()>= criteria[1]) {
    				if(i.getRunsScored()>= criteria[0]) {
    					t.add(i);
    				}
    			}
    		}
