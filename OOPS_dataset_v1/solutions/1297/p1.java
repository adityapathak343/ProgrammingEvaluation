...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		List<Player> storePlayer=new ArrayList<>();
		Scanner inputStream=null;
		try {
			inputStream=new Scanner(new FileInputStream("C:\\Users\\User\\Downloads\\SS_24_25_CBT\\inputCricketData"));
		}
		catch(FileNotFoundException e) {
			System.out.println("inputCricketData not Found");
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
    		outStream=new PrintWriter(new FileOutputStream("C:\\Users\\User\\Downloads\\SS_24_25_CBT\\outputCricketData"));
    	}
    	catch(FileNotFoundException e) {
    	...
    	outStream.println("RunsScore");
    	outStream.println("TeamName");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(int i=0;i<players.size();i++) {
    		try {
    			...
    			System.out.println("No player matches the given name"+e.getMessage());
    		}
    	}
    }
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(int i=0;i<players.size();i++) {
    		if(teamName==players.get()) {
    			playersMatch.add(players(i));
    		}
    	}
    	return playersMatch;
    }

	private Player players(int i) {
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> criteriaMatch=new ArrayList<>();
    	for(int i=0;i<players.size();i++) {
    		if() {
    			criteriaMatch.add(players.get(i));
    		}
    	}
