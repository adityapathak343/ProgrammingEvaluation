...
class RunsComparator implements Comparator<Player> {
    ...
    public int compare(Player p1, Player p2) {
    	...
    	public class ArraySorter{
    		...
    		Arrays.sort(array,(x,y)-> comparator.compare(x,y));
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			file=new FileInputStream(file name);
		}
		Scanner sc = new scanner (file);
		for(int i= 1; sc.hasNext();++i){
			array[i] = sc.next Double();
		}
		sc.close();
    }catch(Exception e){
    	e.printStackTrace();
    ...
    public static void main(String[]args){
    	double[] arr = readScoresArray(filename:"inputCricketData.csv");
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	system.out.println(p:"India");
    	for(int i= 1;i<array.length;i++){
    		if(India = array[i])
    			system.out.println(List<Player> players);
    	}
    	system.out.println(p;"\n");
    	system.out.println(p:"Australia");
    	for(int i= 1;i<array.length;i++){
    		if(Australia = array[i])
    			system.out.println(List<Player> players);
    	}
    	system.out.println(p;"\n");
    	}
    system.out.println(p:"England");
	for(int i= 1;i<array.length;i++){
		if(England = array[i])
			system.out.println(List<Player> players);
	}
