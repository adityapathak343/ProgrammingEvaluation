...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNext()){
			...
			if(!(a==1)){
				...
				String play_name=s[0];
				Role play_role=Role.fromString(s[1]);
				int play_run=Integer.parseInt(s[2]);
				int play_wkt=Integer.parseInt(s[3]);
				String play_team=s[4];
				Player plr=new Player(play_name, play_role, play_run, play_wkt, play_team);
				lst.add(plr);
			}
		}
		inputStream.close();
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		...
    		System.out.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
    		Iterator<Player> itr=players.iterator();
    		while(itr.hasNext()){
    			Player element=itr.next();
    			outStream.println(element.toCsvFormat());
    		}
    	}
    	catch(IOException e){
    		System.out.println(e.getMessage());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) throws Exception{
		...
		while(itr.hasNext()){
			Player element=itr.next();
			if(playerName.equals(element.getPlayerName())){
				element.setRunsScored(element.getRunsScored()+runs);
				element.setWicketsTaken(element.getWicketsTaken()+wickets);
				notfound=false;
			}
		}
		if(notfound){ 
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) throws Exception{
		...
		while(itr.hasNext()){
			Player element=itr.next();
			if(teamName.equals(element.getTeamName())){
				sum+=(double)element.getRunsScored();
				count++;
				notfound=false;
			}
		}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
		...
		while(itr.hasNext()){
			Player element=itr.next();
			if(element.getRole()==Role.fromString("ALL_ROUNDER") && criteria[0]<=element.getRunsScored() && criteria[1]<=element.getWicketsTaken())
				team.add(element);
			}
		return team;
		}
