...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
        ...
        // Step 8: Close the file after reading a.ll data. [1 mark]
        // Step 9: Return the complete list of players. [1 mark]
		  try {
	            // Read data from file
	           Scanner in = new Scanner(new FileReader("inputCricketData.csv");
   ...
   try{
	   inputStream = new Scanner(new FileInputStream("inputCricketData"));
   }
	}
...
while(itr.hasNext()){
	Player name = (Player) itr.next();
if(name.getPlayerName().equals(playerName){
	name.setRunsScored(name.getRunsScored()+ runs);
	name.setWicketsTaken(name.getWicketsTaken()+ wickets);
}
}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	try{
    		...
    		if(pl.getTeamName()== teamName){
    		double sum = 0;
    		sum += pl.getRunsScored();}
    		}
  }
    	{
