...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try{
			...
			while(input.hasNextLine()){
				...
				Player p= new Player(info[0],Role.fromString(((String)info[1])), Integer.parseInt(info[2]), Integer.parseInt(info[3]),info[4]);
				players.add(p);
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> ply= new ArrayList<>();
    	try{int flag=-1;
    		for(Player player:players){
    		if(player.getTeamName().equals(teamName)){
    			ply.add(player);flag=0;
    		}
    	}if(flag==-1) throw new IllegalArgumentException();
...
public class P2022B4A70596P_P1 {
