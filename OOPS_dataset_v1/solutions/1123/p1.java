	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		try {
			...
			while(sc.hasNext()) {
				...
				list.add(new Player(str[0], Role.fromString(str[1]), Integer.valueOf(str[2]), Integer.valueOf(str[3]), str[4]));
			}
			fp.close();
			return list;
		} catch (Exception e) {
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	try {
        	...
        	for(Player p: players) {
        		pw.println(p.getPlayerName()+","+p.getRole().toString()+","+p.getRunsScored()+","+p.getWicketsTaken()+","+p.getTeamName());
        	}
		} catch (Exception e) {
			// TODO: handle exception
			System.err.println(e);
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player p:players) {
    		if(p.getPlayerName().equals(playerName)) {
    			int run=p.getRunsScored()+runs;
    			int wicket=p.getWicketsTaken()+wickets;
    			Role role=p.getRole();
    			String string=p.getTeamName();
    			players.remove(p);
    			players.add(new Player(playerName, role, run, wicket, string));
    			b=true;
    		}
    	}
    	if(!b)
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	for(Player p:players) {
    		if(p.getTeamName().equals(teamName)) {
    			c++;
    			totrun+=p.getRunsScored();
    		}
    	}
    	if(c==0)
    		throw new IllegalArgumentException();
    	return (double)totrun/c;  
