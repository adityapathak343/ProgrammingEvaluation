...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		try 
		{
			w = new Scanner(new FileInputStream("inputCricketData.csv"));
		}
		catch (Exception e)
		{
			System.exit(0);
		...
		while(w.hasNext())
		{
			...
			Role r = Role.fromString(arr[1]);
			int n1 = Integer.parseInt(arr[2]);
			int n2 = Integer.parseInt(arr[3]);

			Player p = new Player(arr[0],r,n1,n2,arr[4]);
			players.add(p);
			j = j+1;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
        ...
        f.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName") ;
        for(Player p:players)
        {
        	String line = p.toCsvFormat();
        	f.println(line);
