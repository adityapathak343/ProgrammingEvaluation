...
* ID Number: 2022B4A70601P
* Name: Siddharth Kamlesh Sharma
* Lab Number: 6118
* System Number: 40
***********************************************************/
...
//		String splitBy = ",";
		...
		while(in.hasNext()) {
			 String name = in.next();
			 Role role = Role.fromString(in.next());
			 int runs = in.nextInt();
			 int wickets = in.nextInt();
			 String team = in.next();
			 
			 Player player = new Player(name, role, runs, wickets, team);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try {
    		...
    		while(itr.hasNext()) {
    			...
    			out.println(current.toCsvFormat());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
		...
		while(itr.hasNext()) {
			Player current = (Player) itr.next();
			try {
				if (current.getPlayerName()==playerName) {
					current.setRunsScored(current.getRunsScored()+runs);
					current.setWicketsTaken(current.getWicketsTaken()+wickets);
				}
			}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
		...
		while(itr.hasNext()) {
			...
			try {
				if(current.getTeamName()==teamName) {
					totRuns+=current.getRunsScored();
				}
			}
			catch(IllegalArgumentException e) {
				e.printStackTrace();
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	ArrayList<Player> belongsToTeam = new ArrayList<>();
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 1: Create an empty list for players matching the criteria. [1 mark]
    	ArrayList<Player> matchesCriteria = new ArrayList<>();
        // Step 2: Go through each player in the list. [1 mark]
    	ListIterator<Player> itr = players.listIterator();
		
		while(itr.hasNext()) {
			Player current = (Player) itr.next();
			if(current.getRole()==Role.ALL_ROUNDER && current.getRunsScored()>=criteria[0] && current.getWicketsTaken()>=criteria[1]) {
				matchesCriteria.add(current);
			}
		}
...
public class P2022B4A70601P_P1 {
