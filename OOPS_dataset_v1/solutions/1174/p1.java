...
class CricketDataHandler {
	 ...
	 * @param Player *********************************/
	public List<Player> readPlayersFromFile(String fileName, Player Player) throws IOException {
    // Question 2: Write code for reading players from a file [Total: 9 marks]
	public	List<Player> playerdetails;// Step 1: Create an empty list to store player details. [1 mark]
     playerdetails = new ArrayList<>();
	((Player) playerdetails).toCsvFormat();  // Step 2: Open the specified file for reading data. [1 mark]
    playerdetails.get(0); // Step 3: Ignore the first line since it contains the column names. [1 mark]
   
    	// Step 4: Read each line one by one until reaching the end of the file. [1 mark]
    
    playerdetails.iterator();
    int i;
       ...
       playerdetails.add(i, Player); // Step 7: Add the new player to the list. [1 mark]
       ((PrintStream) playerdetails).close(); // Step 8: Close the file after reading all data. [1 mark]
       playerdetails.toString(); // Step 9: Return the complete list of players. [1 mark]
    }
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
       ...
       List<Player> player ; player.to // Step 1: Prepare to write data into the specified file. [1 mark]
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	List<Player> play = players;
       for (Player p : play){
    	   if(p.getPlayerName().equals(playerName)) {
    		    int s = p.setRunsScored(runs);
    		       s += runs;
    		     int f =  p.setWicketsTaken(wickets);
    		          f += wickets;
    	   }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
       ...
       double runs = 0;
       int pi = 0;
       if(((Player) play).getTeamName().equals(teamName)) {
    	  for(Player pu : players) {
    		   runs += pu.getRunsScored();
...
class TeamFilterStrategy implements PlayerFilter<String> 
{
    ...
    public List<Player> filter(List<Player> players, String teamName) 
    {
      ...
      List<Player> plays = new ArrayList<Player> (); // Step 1: Create an empty list for players matching the criteria. [1 mark]
       for(Player pi: plays ) 
       { // Step 2: Go through each player in the players list. [1 mark]
         if(pi.getTeamName().equals(teamName))
         {
        	 plays.add(pi);// Step 3: If the player's team matches the given name, add them to the list. [2 marks]
         }
        
        }
       public List<Player> returningPlayers(){
    	   	 return plays;
