...
import RunsComparator.CricketDataHandler;
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile1(String fileName) throws IOException {
		...
		try{
			Scanner in=new Scanner(new File("inputCricketData.csv"));
			in.nextLine();
			while(in.hasNext()){
				...
				list+=new Player p<playerName("%s"),Role("%s"),runsScored("%d"),wicketsTaken("%d"),teamName("%s")>;
				}		in.close();	
		return list;
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		PrintWriter out= new PrintWriter(new FileOutputStream("stuff.txt"));
    		out.println("playerName,role,runsScored,wicketsTaken,teamName");
    		out.println(readPlayersFromFile1("inputCricketData.csv"));
    		out.close();
    		}
    	catch(FileNotFoundException e){
			System.out.println("File not found");
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	try{
			...
			while(in.hasNext()){
				...
				try{
				if(name==playerName){
					int runsScored= in.nextInt()+ runs;
					int wicketsTaken= in.nextInt()+ wickets;
				}}
				catch(IllegalArgumentException e){
					System.out.print("No such player exists.");
				}
    	}
			}
    	catch(FileNotFoundException e){
    	...
    	try{
			...
			while(in.hasNext()){
				...
				try{
					int count=0;
				if(tN==teamName){
					double rS= rS+ runsScored;
					count++;
				}}
				catch(IllegalArgumentException e){
					System.out.print("No such team exists.");
					double avgRuns= rS / count;
					return avgRuns;
				}
    	}
			}
...
public class P2023A70548_P1.java {
    ...
    public static void main(String[] args) {
        ...
        try {
            // Read data from file
            players = dataHandler.readPlayersFromFile1("inputCricketData.csv");
        } catch (FileNotFoundException e) {
