...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(inputStream.hasNextLine()){
			...
			Role r = Role.valueOf(r1);
			int rs = Integer.valueOf(st.nextToken());
			int wt = Integer.valueOf(st.nextToken());
			String tn = st.nextToken();
			Player p = new Player(pname,r,rs,wt,tn);
			players.add(p);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	for(Player player: players){
    		outputStream.println(player.toCsvFormat());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	throw new IllegalArgumentException("no player matched with the given name ");
    }
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> teammem = new ArrayList<>();
    	teammem = t.filter(players, teamName);
    	if(teammem.isEmpty()){
    		throw new IllegalArgumentException("no playeras in the team");
    		
    	}
    	int sum = 0;
    	for(Player p: teammem){
    		sum = sum +p.getRunsScored();
    		
    		
    	}
    	double  avg = sum/teammem.size();
    	return avg;
...
public class P2022B2A71101P_P1 {
