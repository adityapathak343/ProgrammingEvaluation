...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(scanner.hasNextLine()) {
			...
			// System.out.print(lineString);
			 ...
			 Player newPlayer=new Player(lineData[0], Role.fromString(lineData[1]), Integer.parseInt(lineData[2]), Integer.parseInt(lineData[3]), lineData[4]);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter outputPrintWriter=new PrintWriter(new FileOutputStream(fileName));
    	outputPrintWriter.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
    	for(Player player:players) {
    		outputPrintWriter.println(player.toCsvFormat());
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for (Player p:players) {
    		...
    		p.setRunsScored(runs+p.getRunsScored());
        	p.setWicketsTaken(wickets+p.getWicketsTaken()); found=1;break;
        	}
    	}
    	if(found!=1) {throw new IllegalArgumentException("Player to update not found");}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	TeamFilterStrategy filterStrategy = new TeamFilterStrategy();
    	List<Player> teamList=filterStrategy.filter(players, teamName);
    	if (teamList.isEmpty()) {throw new IllegalArgumentException("no player found in the team");}
    	double average;
    	double total=0;
    	double teamSize=(double)teamList.size(); 
    	for (Player p:players) {
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for (Player p:players) {
    		if(p.getTeamName().equals(teamName)) { teamList.add(p);}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
        ...
        // Step 4: Return the list containing all matching players. [1 mark]
    	List<Player> goodPlayers=new ArrayList<Player>();
    	for (Player p:players) {
    		if(p.getRunsScored()>criteria[0]&&p.getWicketsTaken()>criteria[1]&&p.getRole().equals(Role.ALL_ROUNDER)) goodPlayers.add(p);
    	}
    	return goodPlayers;
