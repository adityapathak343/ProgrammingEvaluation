...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		Scanner sc = new Scanner(new FileInputStream("C:\\Users\\user\\Downloads\\SS_24_25_CBT"));
		sc.nextLine();
		while(sc.hasNextLine()){
			String str= sc.nextLine();
			String[] infos= str.split(",");
			String name = infos[0];
			Role role= Role.fromString(infos[1]);
			int runs= new Integer(infos[2]);
			int wickets= new Integer(infos[3]);
			String teamName= infos[4];
			Player p= new Player(name, role, runs, wickets, teamName);
			players.add(p);
			}
		sc.close();
		return players;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	PrintWriter write= new PrintWriter(new FileOutputStream("C:\\Users\\user\\Downloads\\SS_24_25_CBT"));
    	write.println("PlayerName,Role,RunsScored,WicketsTaken,TeamName");
    	for(Player p: players) {
    		String str= p.getPlayerName()+","+p.getRole()+","+p.getRunsScored()+","+p.getWicketsTaken()+","+
    	p.getTeamName();
    		write.println(str);
    	}
    }
