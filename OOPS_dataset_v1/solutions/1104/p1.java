...
* ID Number: 2023A7PS0612P
* Name: Gautam Singhvie
* Lab Number: 6013
* System Number: 19
***********************************************************/
...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(s.hasNext()){
			...
			String[] p1 = np.split(",");
			Player p2 = new Player(p1[0],Role.fromString(p1[1]),Integer.parseInt(p1[2]),Integer.parseInt(p1[3]),p1[4]);
			Players.add(p2);
		}
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	try{
    		...
    		for(Player pl:players){
    			p.println(pl.getPlayerName()+","+pl.getRole().toString()+","+pl.getRunsScored()+","+pl.getWicketsTaken()+","+pl.getTeamName());
    		}
    	}
