...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		//Please store the csv inside the src fle for it to work
		Scanner inputStream = new Scanner(new FileInputStream("./src/cbt_2023A7PS0030P/inputCricketData.csv")); 
		String reader;
		String[] s;
		
		reader = inputStream.nextLine();
		while(inputStream.hasNextLine()) {
			reader = inputStream.nextLine();
			s = reader.split(",");
			Player temp = new Player(s[0],Role.valueOf(s[1]),Integer.parseInt(s[2]),Integer.parseInt(s[3]),s[4]);
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	outputStream.println("PlayerName,RunsScored,WicketsTaken,TeamName");
    	for(Player x : players) {
    		String s = x.getPlayerName()+","+x.getRunsScored()+","+x.getWicketsTaken()+","+x.getTeamName();
    ...
    public void updatePlayerStats(List<Player> players, String playerName, int runs, int wickets) {
    	...
    	for(Player x : players) {
    		if(x.getPlayerName()==playerName) {
    			x.setRunsScored(x.getRunsScored()+runs);
    			x.setWicketsTaken(x.getWicketsTaken()+wickets);
    			return;
    		}
    	}
    ...
    public double calculateTeamAverageRuns(List<Player> players, String teamName) {
    	...
    	List<Player> filteredPlayers = t.filter(players,teamName);
    	for(Player x : filteredPlayers) {
    		sum += x.getRunsScored();
    	}
    	sum /= filteredPlayers.size();
    	return sum;
    }
}
...
class TeamFilterStrategy implements PlayerFilter<String> {
    ...
    public List<Player> filter(List<Player> players, String teamName) {
    	...
    	for(Player x : players) {
    		if(teamName.equals(x.getTeamName())) {
    			filteredPlayers.add(x);
    		}
    	}
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	List<Player> filteredPlayers = new ArrayList<>();
    	for(Player x : players) {
    		if((x.getRunsScored()>=criteria[0])&&(x.getWicketsTaken()>=criteria[1])) {
    			filteredPlayers.add(x);
    		}
    	}
    	return filteredPlayers;
...
public class P2023A7PS0030P_P1 {
