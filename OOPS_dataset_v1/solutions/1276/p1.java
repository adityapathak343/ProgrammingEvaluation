...
class CricketDataHandler {
	...
	public List<Player> readPlayersFromFile(String fileName) throws IOException {
		...
		while(in.hasNextLine()) {
			String name=in.next();
			Role r=Role.fromString(in.next());
			int i= in.nextInt();
			int j= in.nextInt();
			String w= in.next();
			Player p= new Player(name,r,i,j,w);
			list.add(p);
		}
		in.close();
		return list;
    ...
    public void writePlayersToFile(String fileName, List<Player> players) throws IOException {
    	...
    	write.print("PlayerName ");
    	write.print("Role ");
    	write.print("RunsScored ");
    	write.print("Wickets Taken ");
    	write.print("TeamName");
    	for(Player p : players) {
    		write.println(p.toCsvFormat());
    	}
    	write.close();
    }
...
class AllRounderStatsFilter implements PlayerFilter<int[]> {
    ...
    public List<Player> filter(List<Player> players, int[] criteria) {
    	...
    	for(Player p1: players) {
    		if(p1.getRole()==Role.fromString("ALL_ROUNDER")) {
    			list.add(p1);
    		}
    		if(p1.getWicketsTaken()<minw) {
    			minw=p1.getWicketsTaken();
    		}
    	}
    	Comparator<Player> c1= new RunsComparator();
    	Collections.sort(players,c1);
    	Player p= players.getLast();
    	for(Player p3:list) {
    		if(p3.getRunsScored()==p.getRunsScored() && p3.getWicketsTaken()==minw) {
    			list1.add(p3);
    		}
    	}
...
public class F2022B1A70930P_P1 {
